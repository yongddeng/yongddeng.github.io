...
We say a Markov chain is [reversible](https://cims.nyu.edu/~holmes/teaching/asa19/handout_Lecture3_2019.pdf) with respect to $\pi$ if it holds the [detailed balanced condition]() $\pi\_{i}p\_{ij} = \pi_{j}p\_{ji}$ for all $i,j\in\mathbb{S}$. In particular, if $X$ is a general Markov chain, and there exists $\pi$ such that the condition is held, then $\Sigma_{i}\pi\_{i}p\_{ij} = \Sigma_{i}\pi\_{j}p\_{ji} = \pi\_{j}\Sigma_{i}p\_{ji} = \pi\_{j}$, implies that $X$ holds the global balance condition $\pi = \pi{P}$. Consequently, a reversible system is a basis of the sampling algorithm framework called [Markov chain Monte Carlo]() (MCMC) **(#5)** which remedies the features of the [rejection sampling](https://www.youtube.com/watch?v=OXDqjdVVePY), i.e the latter draws samples independently to each other. For instance, the [Metropolis]()'s algorithm uses symmetric conditional candidate densities to return (after a burn-in) sequences of random numbers that reflects a target density.
