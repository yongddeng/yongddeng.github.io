...
Suppose $f\_{i} = P(X\_{k} = i \;\text{for some}\; k>1 \,\vert\, X\_{1}=i)$ is a probability that a Markov chain departed from $i$ returns to $i$ at least once, then $i$ is [recurrent]() if $f\_{i} = 1$, and [transient]() if $f\_{i} < 1$. That is, a recurrent state occurs infinitly often, and a transient state occurs finitly often. We can easily assume that a probability of escaping a transient state $i$ follows $\operatorname{Bernoulli}(1-f\_{i})$, and so a number of returns to $i$ within a $k$-steps follows $\text{Geom}(1-f\_{i})$. Consequently, if $p^{(k)}\_{ii}(s)$ is a probability that a Markov chain departed from $i$ returns to $i$ at the $k$-th step, then $i$ is recurrent if and only if $\Sigma\_{k=1}^{\infty}p^{(k)}\_{ii}(s) = \infty$, and transient if and only if $\Sigma\_{k=1}^{\infty}p^{(k)}\_{ii}(s) < \infty$.
