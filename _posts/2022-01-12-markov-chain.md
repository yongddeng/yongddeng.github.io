...
We say a Markov chain is [reversible](https://cims.nyu.edu/~holmes/teaching/asa19/handout_Lecture3_2019.pdf) with respect to $\pi$ if it satisfies the [detailed balanced condition]() $\pi\_{i}p\_{ij} = \pi_{j}p\_{ji}$ for all $i,j\in\mathbb{S}$. That is, a reversible Markov chain is more restrictive than a staionary one which holds the global balance condition $\pi = \pi{P}$. This particular system is a basis of the general sampling framework so-called [Markov chain Monte Carlo]() (MCMC) **(#5)** which remedies the features of the [rejection sampling](https://www.youtube.com/watch?v=OXDqjdVVePY), i.e. the latter draws samples independently to each other. For instance, if 
