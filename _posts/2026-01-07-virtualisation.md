...
A physical machine (e.g. server) historically ran one application at 10-15% capacity. Whereas, a [virtual machine]() (VM) is a software abstraction of a physical computer with its own virtualised CPU, RAM, disk, and NIC, where a full guest OS boots and runs unmodified, i.e. a [system VM]() virtualising full hardware, unlike a process VM (e.g. JVM, PVM, §602#3.1) which virtualises only a bytecode ISA for a single program. [Virtualisation]() solved it by executing guest code natively on the same ISA, intercepting only sensitive operations, allowing dozens of isolated workloads per machine. While virtualisation is distinct from [emulation](), which translates a foreign ISA entirely in software (e.g. QEMU in TCG mode running an ARM guest on an x86 host), it gave rise to cloud computing via server consolidation and multi-tenancy.
