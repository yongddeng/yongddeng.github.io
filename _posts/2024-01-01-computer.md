...
Historically, interpreters represented source code as ASTs and executed line-by-line using an [recursion](). These trees, while well-suited for program structure representation, were not optimised for execution. That is, types of optimisation were limited, and lowering source code to machine code was also challenging. Interpreters gained widespread adoption among a group of people who prefer the iterative and experimental workflows. In the meantime, compilers remain difficult to be newly developed, despite tons of research and know-how. So, modern interpreter instead compile source code into [bytecode](), and acquired a [virtual machine]() (i.e. VM) for executions.

For instance, [CPython]() (i.e. Python) compiler transforms source code (i.e. .py) into a platform-independent bytecode (i.e. .pyc). [Python VM]() (PVM), the stack-based interpreter, then sequentially (i.e. line-by-line) execute bytecode during runtime by interacting with the OS and underlying hardware. [PEP 3147](https://peps.python.org/pep-3147/) introduced .pyc repository directories, called [\_\_pycache\_\_](), to better organise clutter and enhance cross-version compatibility. The *dis* module can inspect bytecode, revealing the IR generated from source code (i.e. string or script)....  

- <div style="position: relative; display: inline-block; background-color: white;"> <img src="https://peps.python.org/_images/pep-3147-1.png" width="500"> <a href="https://peps.python.org/pep-3147/" target="_blank" style="position: absolute; top: 0px; right: 4px; font-size: 12px;">[src]</a> </div>

JIT compilers dynamically translate "hot region" (i.e. frequently executed code paths) of source code to machine code during runtime, enabling efficient executions.... JIT a part of the PVM in PyPy.... [Omer Iqbal](https://www.youtube.com/watch?v=sQTOIkOMDIw) gave a great presentation about a JIT Compiler.... Nevertheless, other python implementations that equippted JIT is not so popular, and CPython is clearly slower than the other languages....
...
<!-- - <div style="position: relative; display: inline-block; background-color: white"> <img src="../assets/blog/2024-01-01-jitcompile.png" width="500" height="290"> <a href="https://www.youtube.com/watch?v=sQTOIkOMDIw" target="_blank" style="position: absolute; top: 0px; right: 4px; font-size: 12px;">[src]</a> </div> -->
