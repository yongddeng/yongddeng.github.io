...
Cross-compiling has become more accessible with LLVM, but despite extensive research and expertise, creating compilers remains challenging and limits their widespread use. Accordingly, interpreters have gained popularity among users who favour iterative as well as experimental workflows. Modern interpreters convert source code into [bytecode](), which is i) hardware-agnostic: independent of the CPU's [instruction set architecture]() (ISA); and thus ii) executable on any platform (e.g. Linux, macOS, Window) with a compatible [virtual machine]() (VM). Compilation is often faster than -to binary, while a VM executes instruction sequentially, making them highly dynamic.

The [CPython]() compiler's design is outlined in [PEP 339](https://peps.python.org/pep-0339/) (i.e. withdrawn), but in essence, it converts source code (i.e. .py) into bytecode (i.e. .pyc), that consists of [operation codes]() (opcodes) and their corresponding [operands]() (i.e. if any). Given that opcodes are low-level instructions, operands represent various entities depending on the context of opcode, which includes literal constants or immutables (e.g. number, string, tuple), variable or attribute names, or code blocks (e.g. function, class, comprehension). Each Python mnemonic corresponds to an integer of opcode, highly similar to how assembly instructions like *pop*, *push*, and *ret* are represented numerically.
...
The CPython uses the [Python virtual machine]() (PVM), a stack-based interpreter, to execute bytecode at runtime by simulating a CPU's execution model (i.e. stack machine). In particular, the (legacy API-) *PyFrame_New* in Objects/frameobject.c builds a [*PyFrameObject*]() to represent a [stack frame]() for the code being executed. This includes i) *[f_code]()*: a reference to the *PyCodeObject* being executed; ii) *[f_back]()*: a link to the last frame, where these frames form a [linked list]() to be recursively interpreted by the PVM; By default, the function *_PyEval_EvalFrameDefault* in Python/ceval.c serves the main interpreter loop, but as per [PEP 523](), the eval function is configurable.
...
<!-- In summary, the *PyEval_EvalFrameEx* processes opcodes in the context of the given *PyFrameObject*. -->

Note that both code and frame objects are heap-allocated, and so the former acts as a blueprint, while the latter which represents the execution context can also outlive the [function call](). In CPython, a [call stack]() is a pile of *PyFrameObject* pointers modelled by a linked list and stored in stack memory. The *inspect* module helps examining these objects. (i.e. *In recent optimisation, Python 3.11 splits the stack frame object into: the legacy PyFrameObject and the C struct [_PyInterpreterFrame](). Also, Python 3.12, as proposed in [PEP 709](https://peps.python.org/pep-0709/), inlines list, dict, and set comprehensions into the code rather than compiling them as nested functions that isolate iteration variables*).
