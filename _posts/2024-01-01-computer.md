...
The [word length]() of the CPU represents the number of bits it can process in a single operation (e.g. 32-bit, 64-bit). It dictates the size of registers and the width of data bus. Additionally, the [instruction set architecture]() (ISA) is a simplified but sufficient definition of the hardware interface, which describes i) the abstract behaviour of the underlying compute engine; and ii) what fundamental operations can be encoded in machine code instructions; It will define what sort of registers the processor has, how they interact with memory, and what arithmetic operations are possible. Instructions will be given a mnemonic code: {e.g. *ADD*: {Hex: 8, Semantic: A $\gets$ A $+$ B}}.

A large word length allows it to process more data at once or address larger memory spaces. Below is a demo calculator (i.e. 4-bit computer) that can only perform $2^4 = 16$ unique instructions on tiny chunks of data (i.e. 4 bits at a time). A 32-bit system is by far more powerful as it can define milions of operations and address 4 GB (i.e. or more with segmentation). Note that using types that match the CPU's native word length can often result in efficient computations, but programming languages including C define the [data type]() size only by minimum standards {e.g. *char*: 1 byte, *int*: 4 btyes} to ensure portability across diverse CPU architectures.

<!-- Modern CPUs usually use techniques such as [pipelining]() and [branch prediction]() to maximise instruction throughput.  -->
...
<!-- http://cpuville.com/Projects/Original-CPU/programming.html -->
...
An instance of a program, called a [process](), resides in RAM while running. Roughly speaking, local variables and function parameters are allocated in the [stack]() which operates in a LIFO manner. The stack size is determined at compile time, and the [stack pointer]() (SP) tracks the addresses of [stack frames](), created by [function calls](https://www.youtube.com/watch?v=cSSpnq362Bk), being pushed to the [call stack](). In contrast, dynamically allocating variable-length data (e.g. arrays) in the [heap]() requires sending a [system call]() to the OS to obtain a memory chunk. While the stack is fixed in size and offers faster memory access via the proximity, heap allocations are more prone to cache misses and fragmentation.

<!-- An instance of a program, called a [process](), resides in RAM while running. Roughly speaking, __________ ______ are allocated in [stack](), which behaves under the LIFO manner. The size of stack is determined at compile time. The [stack pointer](https://www.youtube.com/watch?v=ioJkA7Mw2-U) (SP) tracks the addresses of [stack frames]() created by each [function call]() and being pushed to the [call stack](). In contrast, one must send a [system call]() to the OS and recieve a memory chunk to dynamically allocate variable-length data (e.g. array) into [heap](). It has higher overheads and cache misses due to a number of system calls and unpredictable allocation patterns, while a size of stack is fixed upon compilation and hence faster.  -->
...
- <div style="position: relative; background-color: white; display: inline-block;"> <img src="https://www-user.tu-chemnitz.de/~heha/hsn/chm/avr-libc.chm/malloc-std.png" width="500" height="250"> <a href="https://www-user.tu-chemnitz.de/~heha/hsn/chm/avr-libc.chm/malloc.html" target="_blank" style="position: absolute; bottom: -8px; right: 4px; font-size: 12px;">[src]</a> </div>
