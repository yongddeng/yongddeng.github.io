...
[Alban Desmaison](https://pytorch.org/docs/stable/community/persons_of_interest.html) noted: "When Torch7, originally written in [Lua](), rely on the C/C++ libraries: Tensor Handling (TH/THNN) and Tensor Handling CUDA (THC/THCUNN) for CPU/GPU computing, where THC* had CUDA/cuDNN dependencies, i) the Initial PyTorch was a Python wrapper + new autograd engine + TH*; ii) Aten was developed to provide C++ interfaces, thus adding new functionalities (i.e. tensor operations) to PyTorch occurs within ATen; iii) [LibTorch]() is the C++ distribution of PyTorch built as a replacement/supplmenet for the Python wrapper (which used [Pybind11]() for wrapping ATen). Its users can directly exposes the C++ API for high performances".

<!-- https://medium.com/@pouyahallaj/libtorch-the-c-powerhouse-driving-pytorch-ee0d4f7b8743 -->

Facebook utilised LibTorch and added *torch.jit.trace* to support JIT compilation. It records operations during a forward pass with fixed inputs, producing a SCG that is ideal for models with predictable execution paths (i.e. control flows may break compilation). They also added *torch.jit.script* which directly compiles Python code, allowing input-dependent models. Any [TorchScript]() program can be saved from a Python process and loaded in a process where there is no Python dependency (i.e. train.py $\to$ deploy.cpp)

However, if both APIs are insufficient, models can be exported to the [Open Neural Network Exchange]() (ONNX), a standardised format for cross-platform deployment, while dynamic models may need adjustments to conform to ONNX's static graph requirements.
