...
Specialised processors offload computational workloads from the CPU, enhancing performance for specific tasks. GPUs are currently the most widely recognised accelerator, although [application-specific integrated circuits]() (ASICs) offer greater efficiency despite of its minimal programmability. Notable ASIC-based hardware includes AWS Inferentia (Inf1) and Trainium, Google's TPU, and Apple's neural engine (ANE). As programs relying on specific accelerators are hardware-dependent and require compilation for the target machinery, a set of unsupported operations (e.g. a custom or rare activation function within a ML model) are executed on the CPU as a [fallback](https://www.quora.com/What-is-CPU-fallback).
