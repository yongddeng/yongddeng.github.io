...
<!-- Underpinning *torch.compile*, a feature that pushes [PyTorch 2.0](https://pytorch.org/assets/pytorch2-2.pdf) (released in 2023) performance to new heights and starts the move for parts of PyTorch from C++ back into Python, are new technologies – TorchDynamo ,,, -->

[TorchDynamo]() (i.e. *torch._dynamo*) is the tracer used in [*torch.compile*](), leveraging the [FX]() (i.e. Python-to-Python transformation toolkit) to produce clean graphs that are compatible with downstream compilers. During graph acquisition, TorchDynamo [hooks]() into Python's bytecode execution via the [Frame Evaluation API]() of CPython, and calls *torch.fx.symbolic_trace* to simulate execution with [proxies]() and identify capturable regions within a program, which then become nodes of an [FX graph](). Dynamo breaks a computation graph when it encounters unsupported Python features (e.g. data-dependent control flow) for CPython (i.e. default interpreter) to run them instead.


<!-- The python code gen records captured operations as nodes in the FX graph.  -->

Apparently, *torch.compile* has the [*fullgraph (bool)*]() parameter. With its default backend (i.e. Inductor), if the argument is *False (default)*, Dynamo searches compileable regions in the function for optimisation. If set to *True*, the entire function must be capturable as a single graph. If it fails due to graph breaks, an error will be raised, whereas other compilers might either raise errors or fail silently....

 <!-- *torch._dynamo.explain* and *TORCH_LOGS=graph_breaks* support debugging, TorchDynamo, which achieves 9x% of operation capturing, stays at Python-level IR to keep the dynamic nature of the language.... -->

<!-- To skip compilation, you can decorate the offending function with *@torch.compiler.disable*.... All the existing APIs are applied at the TorchDynamo level. Therefore, these APIs have visibility to only what TorchDynamo sees. This can lead to confusing scenarios. Eg1., torch._dynamo.disallow_in_graph will not work for ATen operators because they are visible to AOT Autograd. Eg2., torch._dynamo.disallow_in_graph(torch.ops.aten.add) will not work in the above example. -->

<!-- which can then be further optimised -->


<!-- press "cmd + ." with some lines of code-->

<!-- In short, symolically evaluate bytecode... 이후 gm을 arg로 받는 compiler를 context manager: [*torch.dynamo.optimize*]()로 감싸서 타겟 kernel에서 추출한 그래프 data structure에서 ir로 변환... -->
  
  <!-- When encountering unsupported Python features, previous compilers (e.g. TorchScript) either raise an error or silently fail. TorchDynamo, on the other hand, will break the computation graph (e.g. it extracts two distinct graphs for an if statement). -->


<!-- The Graph is ready for optimisation or execution by back-end compilers such as TorchInductor, TensorRT, or XLA. Unlike static tracing methods (e.g., TorchScript), TorchDynamo operates transparently in eager mode, allowing users to write standard Python code without modifications while benefiting from graph-based optimisations. -->

<!-- Moreover, the above components can be used separately. For example, symbolic tracing can be used in isolation to capture a form of the code for analysis (and not transformation) purposes. Code generation can be used for programmatically generating models, for example from a config file. -->
...
- TorchDynamo is a generalisation of torch.jit.trace
  ...
  - PyTorch 2.0의 default back-end는 Triton이고.. fx에서 IR로 lowering을 마친 ops을 Inductor가 fusion하여 Triton의 kernel로 표현함. -->
...
[AOTAutograd](), as it was before, generates a backward computation graph.... [TorchInductor]() then consumes to yield optimised kernels.... 
