...
Computing begins with machine language, the lowest level, consisting of [binary]() instructions interpretable and executable by the CPU. [Assembly]() languages, such as x86, ARM, and MIPS, introduced human-readable mnemonics which are translatable into machine code. Low-level languages (i.e. farther from hardware) like C offer higher abstraction while maintaining hardware control, and use [compilers]() to translate the [source code]() into executable machine code. The evolution continued with high-level languages like Python, Java, and C++. They even abstract hardware complexities and use compilers or [interpreters]() (i.e. may or may not use the JIT compilation stack).
