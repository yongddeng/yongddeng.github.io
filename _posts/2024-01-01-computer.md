...
A [*torch.autograd.Function*]() is used to define non-native PyTorch operations which require custom forward and backward computations, although PyTorch provides various built-in Function classes (e.g. AddBackward0, MulBackward0) which handle [gradients]() for standard operations. It creates a new DCG for each execution of an operation on tensors. That is, *torch.Tensor* and *torch.autograd.Function* can be respectively considered as nodes and edges of a graph which carries data and encapsulates the logic for bidirectional calculations, facilitating the dynamic propagation across a neural network (NN) at run times. One could use *Tensor.register_hook* to act with flows.
