...
Compared to OG interpreters which traversed ASTs recursively during runtime and suffered from slow executions, the introduction of a VM and bytecode input significantly improved performance and also portability. ASTs were well-suited only for representing program structure as they were not optimised for efficient execution. In contrast, a lower-level IR enables more sophisticated optimisations and custom implementations. For instance, [PEP 3147]() introduced the .pyc repository directory, called [\_\_pycache\_\_](https://peps.python.org/pep-3147/), to better organise files and improve cross-version compatibility. CPython skips recompilation if an up-to-date .pyc file is present in the directory.
...
The introduction of JIT compilers aims to further accelerate execution speed. Given that AOS (i.e. general-purpose) compilation could be slow, as [Omer Iqbal](https://www.youtube.com/watch?v=sQTOIkOMDIw) mentioned in his solid presentation, traditional JIT compilers, found in other languages such as Java (JVM) or JavaScript (V8 engine), employ multi-tiered optimisation strategies, such as the 'warm' or 'hot' stages, which identify more frequently executed code paths. These code regions are then compiled into machine code during runtime. While CPython, the reference implementation of Python, does not have such a feature, one can find other Python implementations incorporating JIT compilation.
