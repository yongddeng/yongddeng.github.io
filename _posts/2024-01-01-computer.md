...
In fact, TS does not support certain libraries or Python features (e.g. generator, comprehension), instead forces incompatible modules to be either wrapped/avoided. Also, TS-compiled models naturally loss some of eager charm (i.e. PT's eager execution) flexibility and dynamic behaviour. The [*torch.compile*]() API is therefore introduced as a domain-specific compiler of [Pytorch 2.0]() (PT2). On the surface, PT2 converts PyTorch-related regions into a SCG (as TS does), but it can also continue to interpret the remaining (i.e. general) Python code as usual, maintaining its "pythonic" nature. The dynamic trace (unlike "once and for all" of TS) on a forward pass distinguishes it.
...
Explicitly, \_\_torch_dispatch\_\_ is .... This is a diagram of how torch_dispatch might work with vmap, where the dotted arrows represent paths that could have been take. \_\_torch_dispatch\_\_ is the only way to go from C++ back into Python.... 

<!-- Note that AOTAutograd compiles computations (prior to execution) within the dynamic context of TorchDynam, so the term ahead-of-time is relative to the specific computation being intercepted and optimized at runtime -->
