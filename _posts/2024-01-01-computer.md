...
A computer memory is essentially an array of bytes each with a unique address. That is, [random access memory]() (RAM), register, and cache are all some abstractions. The latency of  [dynamic RAM]() (DRAM) can be estimated as $T_{\text{precharge}} + T_{\text{row activation}} + T_{\text{col. access}}$. The access is indeed fastest if data is in an active row (i.e. contiguous), although the physical design of memory cells heavily matters. [Static RAM]() (SRAM) is better suited for cache memory due to its high speed and stability, despite its higher cost. A disk can also be used as [virtual memory](), via [paging]() or [segmentation](), to free up RAM for a process. However, excessive swapping leads to [thrashing]().
...
I/O devices allow beginners to manipulate complex systems without requiring deep knowledge of binary encoding or CPU instructions. For instance, {I: 'keyboard', O: 'monitor'}, further facilitate communication between a computer and its external environment. Each key press is converted into a corresponding binary using encoding standards. In particular, the [unicode transformation format-8]() (UTF-8) is a variable-length standard representing over a million codepoints as sequences of 8-bit bytes. It is a superset of all characters in widespread use today, including those defined by [Hex]() and [ASCII]() (i.e. 7-bit encoding standard merely supporting 128 English letters).

Whereas, programmers directly interact with computers by coding. At the lowest level, computing begins with machine code consists of [binary]() instructions that are CPU interpretable and/or executable. [Assemblies]() (e.g. x86, ARM, MIPS) rather layered human-readable mnemonics to be translated into machine code. Low-level languages like C offer even higher abstraction while maintaining hardware control, but they rely on [compilers](https://www.reddit.com/r/ProgrammingLanguages/comments/1475h9o/how_to_make_a_compiler/) to convert [source code]() into an [executable](). The evolution continued with high-level languages such as Python, Java, and C++. They abstract away hardware complexities and use compilers or [interpreters]() (i.e. may or may not use JIT).
