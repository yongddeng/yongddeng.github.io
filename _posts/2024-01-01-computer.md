...
A front-end compiler often generates an IR through i) lexical analyser (Lexer): breaks the [source code]() into tokens (e.g. identifier, literal, and keyword); ii) parser: builds an [abstract syntax tree]() (AST) on the structure of the tokens and the language's grammar to represent the hierarchical structure of the source code; iii) semantic analyser: ensures correctness of the program beyond just syntax (e.g. type checking, variable declarations, and scope resolution); and iv) IR generator: transforms the validated source code into a universal IR; Modern C compilers (e.g. GCC, and Clang) are [self-hosting]() (i.e. written in C/C++) and were developed via a [bootstrapping]() process.

In practice, an IR generated by a front-end gets optimised (e.g. dead code elimination) by the [LLVM Core]() library. A back-end will either compile this optimised IR [ahead-of-time]() (AOT) (i.e. *llc* cmd), or execute directly using a [just-in-time]() (JIT) compilation interpreter (i.e. *lli* cmd). The expressive power of the IRs and their ability to understand the high-level semantics determine sophistications of a compiler. That is, it was natural for most high-level languages to develop their own AST for associated infrastructures (e.g. Swift-SIL, Rust-MIR, Pytorch-TorchScript), resulting various modules for similar tasks to unnecessarily co-exist as in language- and/or domain-specific IRs.
