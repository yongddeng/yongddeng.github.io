...
I/O devices (e.g. {I: 'keyboard', O: 'monitor'}) facilitate communication between a computer and its external environment by converting human actions into [binary]() which the CPU can process. For instance, each key press on a keyboard is translated into a corresponding binary using encoding standards. The [unicode transformation format-8]() (UTF-8) is a variable-length encoding standard that represents characters as sequences of 8-bit bytes, allowing for over a million distinct codepoints. It s a superset of all characters in widespread use today, including those defined by [ASCII]() (i.e. a 7-bit encoding standard that only defines 128 characters primarily for English text). 

- <div style="position: relative; display: inline-block;"> <img src="https://i.sstatic.net/opk4z.png" width="500" > <a href="https://stackoverflow.com/questions/75348835/why-spc-can-resync-characters-in-asynchronous-serial-output" target="_blank" style="position: absolute; top: 0px; left: 4px; font-size: 12px;">[src]</a> </div>
...
Having specialised processors can offload CPU. A GPU is the current most famous processor. An [application specific integrated circuit]() (ASIC) is not normally programmable and does not expose developer-friendly API, but can be more efficient than GPUs. Intel's Gaudi offers programmability but is less versatile than a GPU. There are AWS Inferentia (inf1) and Trainium,  Google TPU, and Apple ANE. inf1 based on the ASIC offers a list of supported operations it can accelerate, and implements CPU fall back mode for the operations being part of a given ML model. A program is hardware dependent, requires a compilation to an executable of a target machine...
<!-- 
...
[Alban Desmaison](https://pytorch.org/docs/stable/community/persons_of_interest.html) noted: "When Torch7, originally written in [Lua](), rely on the C/C++ libraries: Tensor Handling (TH/THNN) and Tensor Handling CUDA (THC/THCUNN) for CPU/GPU computing, where THC* had CUDA/cuDNN dependencies, i) the Initial PyTorch was a Python wrapper + new autograd engine + TH*; ii) Aten was developed to provide C++ interfaces, and thus an implementation of new functionalities for PyTorch were done within ATen; iii) [LibTorch]() is the distribution of PyTorch, a replacement and/or supplmenet for the Python wrapper (which used [Pybind11]() to wrap ATen), enabling direct access to the core backends through the C++ API".
...
- <div style="position: relative; display: inline-block;"> <img src="https://miro.medium.com/v2/resize:fit:1400/1*RjhrtZC4W5eftvUUARb-Gw.png" width="500" height="455"> <a href="https://www.tensorflow.org/mlir/overview" target="_blank" style="position: absolute; top: 0px; left: 4px; font-size: 12px;">[src]</a> </div>
