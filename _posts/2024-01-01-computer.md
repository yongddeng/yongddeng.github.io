...
[TorchDynamo]() (i.e. *torch._dynamo*) is the tracer, used in *torch.compile*, leveraging the [FX]() (i.e. Python-to-Python transformation toolkit) to produce clean graphs that are compatible with downstream compilers. For graph acquisition, Dynamo [hooks]() (i.e. intercepts) Python bytecode execution using the [Frame Evaluation API]() of CPython, and calls *torch.fx.symbolic_trace* to simulate execution with [proxies]() and identify capturable regions within a program, which become nodes of an FX graph. Dynamo breaks a computation graph when it encounters unsupported Python features (e.g. data-dependent control flow), that CPython (i.e. default interpreter) can execute them.
