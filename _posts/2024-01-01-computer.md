...
[Alban Desmaison](https://pytorch.org/docs/stable/community/persons_of_interest.html) noted: "When Torch7, originally written in [Lua](), rely on the C/C++ libraries: Tensor Handling (TH/THNN) and Tensor Handling CUDA (THC/THCUNN) for CPU/GPU computing, where THC* had CUDA/cuDNN dependencies, i) the Initial PyTorch was a Python wrapper + new autograd engine + TH*; ii) Aten was developed to provide C++ (cpp) interfaces, thus adding functionalities (i.e. tensor operations) to PyTorch can be done in ATen; iii) [LibTorch]() is the (standalone) C++ distribution of PyTorch built as a replacement for the Python wrapper (which used [Pybind11]() for wrapping C++ libraries), and thus its users can directly exposes the C++ API".
