...
Different types of cores within GPU are responsible for ..... However, the equal-sized set of hardwares may differ in performances. For example, GTX 980 Ti(Maxwell architecture)와 GTX 980 Ti(Pascal architecture)의 경우는 비슷한 수의 CUDA Core와 Transistor가 존재하지만 트랜지스터의 크기가 성능차를 만듬. 더 작은 Transistor를 사용하는 Pascal architecture의 전력소비 감소로 더 많은 트랜지스터가 사용가능했고, max clock frequency가 향상되어 단일 Core가 더 빠르게 작업을 수행하는 식으로 두 설계구조 간의 성능차를 만듬. The below is a simplified JSON representation of the NVIDIA's Ampere architectures (A100 40GB Ver.) with modules commonly found in their recent products:
    ...
    GA100: { 
                                            ...
                                            "streaming_multiprocessor": [  # i.e. repeat for 2 SMs/TPC 
                                                    {
                                                            ...
                                                            "tensor_cores": 4,
                                                            "warp_schedulers": 4,
                                                            "on_chip_memories": {"registers": 256 KB/SM, "L1_cache/SMEM": "192 KB"}
                                                    },
                                            ]
                                    },  
                            ]
            ...
            memory_hierarchy: {
                    "VRAM": {"interface": "5120-bit HBM2", "capacity": "40 GB", "bandwidth": "1.6 TB/s"},
                    "L2_cache": "40 MB"
            },
