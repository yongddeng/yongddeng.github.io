...
If a graph break occurs, non-torch.\* (e.g. printing, logging, tensor-dependent conditions) fall back to the default interpreter, bypassing the graph representation. Therefore, reducing breaks is crucial in PT2 (i.e. dynamic-to-static compilation framework) as they degrade performance. Apparently, *torch.compile* has the *fullgraph* parameter, controlling the granularity of tracing. When set to *False (default)*, it attempts to discover compileable regions in the function. If set to *True*, the entire function must be capturable into a single graph. Other APIs, such as *torch._dynamo.explain* and *torch.fx.graph_module.GraphModule.print_tabular*, are helpful for diagnosing breaks.
