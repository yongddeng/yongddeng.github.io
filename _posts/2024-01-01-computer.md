...
[TorchDynamo]() (i.e. Dynamo) serves as the front-end, leveraging PEP 523 (i.e. frame evaluation API) and the [functional transformation]() (FX) framework to represent Python code as high-level DCGs compatible with downstream compilers. For graph acquisition, Dynamo i) [hooks]() into bytecode execution using PEP 523; ii) analyses bytecode and identifies torch.\* and non-torch.\* bits of a program; and iii) convert compilable regions into a graph; *torch.fx.symbolic_trace* can be used with [proxies]() to simulate captured torch.\* operations and record them as nodes in an [FX graph](), while general Python code segments in non-torch.* fall back to CPython (default interpreter).

Also, [graph breaks]() occur when Dynamo encounters complex behaviours. For example, [dynamic control flow]() refers to situations where the path of execution (which operation to apply) depends on runtime data. Therefore, if one has the *condition = lambda x: x.mean() > 0.5* that relies on the values held in *x: torch.Tensor*, then the code block *if condition(x): return x * 2; else: return x / 2;* are broken down into two FX graphs, leading Dynamo to trace the computation separately for each case. Frequent graph breaks mean [performance hit](), and APIs like *torch._dynamo.explain* and *torch.fx.graph_module.GraphModule.print_tabular* are helpful for diagnosing and debugging. <!-- Developers can also catch graph breaks early by wrapping function calls in try-except blocks. -->
...
<!-- A [graph break]() occurs when it encounters operations it cannot support during dynamic tracing. If broken, non-torch.\* (e.g. printing, logging, tensor-dependent conditions) fall back to the default interpreter, bypassing the graph rep.... Reducing breaks is therefore important in PT2 (i.e. dynamic-to-static compilation framework) because part of the model execution will be traced separately. -->

Apparently, *torch.compile* has the *fullgraph* parameter that controls the granularity of tracing. When set the default *False*, it attempts to discover compileable regions in the function. If set to *True*, the entire function must be capturable into a single graph.
