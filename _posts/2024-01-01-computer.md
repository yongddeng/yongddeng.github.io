...
Ultimately, JIT compilers appeared to further accelerate execution speed. Given that AOS (i.e. general-case) compilation can be very slow, as Omer Iqbal mentioned in his [presentation](https://www.youtube.com/watch?v=sQTOIkOMDIw), traditional JIT compilers found in languages such as Java (JVM) or JavaScript (V8 engine) employ multi-tiered optimisation strategies. These tiers, such as the 'warm' or 'hot' stages, efficiently identify and categorise frequently executed code paths. The code regions are compiled into machine code during runtime. While CPython, the reference implementation of Python, does not have such a feature, one can find other Python implementations incorporating JIT compilation.
