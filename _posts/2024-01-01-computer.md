...
FX graphs (i.e. of torch.\*) produced by Dynamo accomodate guards and so the downstream compilers find them difficult to work with. Also, the nodes in FX graphs (i.e. tensor operations) must have *grad_fn* to be *.backward* callable, while the backward pass is generally more expensive and tightly coupled to the efficiency of the graph representing the forward pass. In the same vein, [AOTAutograd]() (i.e. AOT) decomposes all IR nodes into functional, ATen operators being closer to hardware. AOT traces the forward and backward passes as a joint graph, and then partitions the forward (i.e. FWD) and backward (i.e. BWD) graph into two separate FX graphs (i.e. of aten.*). 

<!-- Graph lowering via the lower-level modules is a way to generate a more efficient, minimal BWD. -->

[PrimTorch]() (i.e. torch/_prims) is newly developed as a minimal abstraction of ATen specific for graph-based workflows. It reduced ~2k+ PyTorch operators down to a closed set of ~250 primitive operators.... Moreover, unlike Dynamo's symbolic tracing, AOT uses [*\_\_torch_dispatch\_\_*]() to extract both forward and backward graphs, and then wraps them in *torch.fx.GraphModule* containers.... AOTAutograd can employ techniques like activation checkpointing or recompute-on-demand to manage this memory-pressure tradeoff....
...
- <div style="position: relative; display: inline-block;"> <img src="https://pytorch.org/assets/images/training-production-ai-models/blog-fig1.jpg" width="500" height="140"> <a href="https://pytorch.org/blog/training-production-ai-models/?utm_source=social-facebook&utm_medium=m4d&utm_campaign=organic&utm_content=post-url&utm_offering=artificial-intelligence&utm_product=training-production-ai-models_01302024&eventSource=organicsocialm4d&content_id=FmvhyYO3MCBLU8r" target="_blank" style="position: absolute; bottom: -8px; right: 4px; font-size: 12px;">[src]</a> </div>
