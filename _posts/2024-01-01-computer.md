...
[Script mode]() can acheive higher throughputs at inference. So, they added [TorchScript]() (TS) on top of LibTorch to ace deployments, and preserve clean UX via the compilation APIs: [*torch.jit.trace*]() and [*torch.jit.script*](). The former traces a model on some [proxies]() and records tensor operations while the latter parses the entire source code, both of which build a SCG (i.e. capturing regions may differ) as an TS IR. An TS-compiled Python program can be either executed at the PT JIT runtime, or, exported into non-Python environments such as the [Open Neural Network Exchange]() (ONNX) format that others often comply with to complete "train.py $\to$ deploy.cpp" framework. 
...
<!-- TS also loses eager as one has to TSify a model to run it -->
...
The compiler of CPython compiles source code into [bytecode]() (i.e. .pyc). A [CodeObject]() is generated during the compilation process to store the bytecode instructions along with static data (e.g. consts list, variable names, exception table) required for execution. As the *PyEval_EvalCode* function in [cpython/Python/ceval.c]() executes a CodeObject, a [PyFrameObject]() is formed to hold the runtime state for the execution, where its *f_code* attribute points to the corresponding CodeObject (i.e. the CodeObject acts as a blueprint). By default, the *PyEval_EvalFrame* calls *_PyEval_EvalFrameDefault* for frame execution, but as per [PEP 523](), the evaluation function is configurable.
...
FX graph (i.e. torch.* calls) produced by Dynamo  is specialised for guards. Hence they are still nontrivial (e.g. default arguments) and can have mutating operations (e.g. add_) which downstream compilers find difficult to work with. [AOTAutograd]() (i.e. AOT) deals with these complicated interactions, to lower the barrier between Python code and the inner backends, by canonicalising all IR nodes into functional ATen operators. Given that ATen is the implementation source for most tensor operations in PyTorch, [PrimTorch]() (i.e. torch/_prims) is a minimal abstraction of ATen specific for graph-based workflows, allowing better traceability and downstream optimisation (e.g. operation fusion, redundant computation elimination)

In particular, PrimTorch canonicalises 2k+ PyTorch operators down to a closed set of ~250 primitive operators that developers can target to build a complete PyTorch backend. PrimTorch operations may ultimately call into ATen kernels for execution.... Dynamo is a Python-level JIT compiler designed to capture and transform a PyTorch program dynamically into FX-based IRs. It excels at capturing the entire program but only accelerates a forward pass (FWD). AOT optimises a backward pass (BWD) by decomposing AD computations into smaller, lower-level primitives. The backward computations are generally more expensive, and its performance is naurally and tightly coupled to the efficiency of the forward graph. Graph lowering via the lower-level modules is a way to generate a more efficient, minimal BWD.

<!-- Unlike Dynamo's symbolic tracing, AOT uses [*\_\_torch_dispatch\_\_*]() to extract both forward and backward graphs, and then wraps them in *torch.fx.GraphModule* containers.... -->

<!-- Ahead-of-time backward pass (i.e. a SCG construction) is also in contrast to the PT 1.x's approach where the backward pass is dynamically generated during runtime. -->
