...
The CPython uses the [Python virtual machine]() (PVM), a stack-based interpreter, to execute bytecode at runtime by simulating a CPU's execution model (i.e. stack machine). In particular, the (legacy API-) *PyFrame_New* in Objects/frameobject.c builds a [*PyFrameObject*]() and models a stack frame for the current code being executed. This object mainly contians i) *[f_code]()*: a reference to the *PyCodeObject* being executed; ii) *[f_back]()*: a link to the previous frame, where frames form a [linked list]() that the PVM manages via recursion; By default, the *_PyEval_EvalFrameDefault* in Python/ceval.c serves the main interpreter loop, but as per [PEP 523](), the eval function is configurable.
 
 <!-- while the [*PyEval_EvalFrameEx*]() function (i.e. [*PyEval_EvalFrameDefault*]() in Python 3.9+), located in Python/ceval.c, is the main interpreter loop. -->
