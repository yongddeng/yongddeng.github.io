...
[Script mode]() can achieve higher throughputs at inference. So, they added [TorchScript]() (TS) on top of LibTorch to ace deployments, but preserve clean UX via the compilation APIs: [*torch.jit.trace*]() and [*torch.jit.script*](). The former traces a model on some [proxies]() and records tensor operations while the latter parses the entire source code, both of which build a SCG (i.e. capturing regions may differ) as an TS IR. An TS-compiled Python program can be either executed at the PT JIT runtime, or, exported into non-Python environments such as the [Open Neural Network Exchange]() (ONNX) format which others shall comply with to implement "train.py $\to$ deploy.cpp" framework. 
...
In fact, TS does not support certain libraries or Python features (e.g. generator, comprehension), instead forces incompatible modules to be either wrapped/avoided. Also, TS-compiled models naturally loss some of eager charm (i.e. PT's eager execution) flexibility and dynamic behaviour, To overcome these limitations, the [*torch.compile*]() API was introduced as a domain-specific compiler of [Pytorch 2.0]() (PT2). On the surface, PT2 transforms PyTorch-specific regions into a SCG as TS does, but it can continue to interpret the remaining (i.e. arbitrary) Python code as usual, maintaining its "pythonic" nature. The new graph acquisition stack relies on features of CPython.
...
The compiler of CPython compiles source code into [bytecode]() (i.e. .pyc). A [CodeObject]() is generated during the compilation process to store the bytecode instructions along with static data (e.g. consts list, variable names, exception table) required for execution. As the *PyEval_EvalCode* function in [cpython/Python/ceval.c]() executes a CodeObject, a [PyFrameObject]() is formed to hold the runtime state for the execution, where its *f_code* attribute points to the corresponding CodeObject (i.e. the CodeObject acts as a blueprint). By default, the *PyEval_EvalFrame* calls *_PyEval_EvalFrameDefault* in frame execution, but as per [PEP 523](), the evaluation function is configurable.

[TorchDynamo]() (i.e. Dynamo) is the front-end compiler designed for Python-level graph acquisition. It leverages PEP 523 (i.e. the frame evaluation API) and [functional transformation]() (FX) toolkit. Dynamo initially i) hooks into bytecode execution; ii) analyses bytecode with proxies and identifies torch.\* and non-torch.\* calls; and iii) convert compilable regions into an [FX graph]() (i.e. Torch IR); Assume that the mock .py had: {[torch.* ops, torch.tensor]: TorchVariable, Py\_builtin\_variables: BuiltInVariable, Py\_lists: ListVariable}, then operations on a [TensorVariable]() adds a FX node in the graph, and *torch.fx.symbolic_trace* (or, *torch._dynamo.export*) could illustrate an FX graph.
