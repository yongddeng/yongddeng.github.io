...
[TorchDynamo]() (i.e. Dynamo) serves as the front-end designed for Python-level graph acquisition. It leverages both PEP 523 (i.e. frame evaluation API) and [functional transformation]() (FX) toolkit. Dynamo initially i) hooks into bytecode execution; ii) analyses bytecode with some proxies and identifies torch.\* and non-torch.\* calls; and iii) convert compilable regions into a graph; *torch.fx.symbolic_trace* (or, *torch._dynamo.export*) can be used to produce an [FX graph]() in which operations on a [TensorVariable]() adds a FX node in the graph. For example, the .py file may have {[torch.* ops, torch.tensor]: TorchVariable, Py\_builtin\_variables: BuiltInVariable, Py\_lists: ListVariable}.
