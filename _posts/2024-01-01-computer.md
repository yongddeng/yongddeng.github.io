...
APIs such as *torch._dynamo.explain* or *torch.fx.graph_module.GraphModule.print_tabular* help visualise and diagnose graph structures and breaks during debugging. One can also wrap function calls in a try-except block to enforce strict behaviour and catch graph break exceptions early, aiding in the identification of performance bottlenecks in the Python-level JIT compiler. Apparently, the *torch.compile* takes a parameter *fullgraph (boolean)* which controls the graph-capturing behaviour. If the argument is *False (default)*, it identifies and compiles capturable regions in the function. If set to *True*, the entire function must be compiled as a single, uninterrupted graph. 
...
- res ... 
  ...
  <div style="position: relative; display: inline-block;"> <img src="../assets/blog/2024-01-01-torchdynamo.png" width="500" height="420"> </div>
...
*torch.compile* wraps around lower-level tools other than *torch._dynamo*.
