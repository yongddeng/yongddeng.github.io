...
At last, the *_PyEval_EvalFrameDefault* in Python/ceval.c serves as the main interpreter loop, but, as per [PEP 523](), it is configurable. "All" Python objects and data structures are heap-allocated by construction. A *PyCodeObject* act as a blueprint and a *PyFrameObject* holds the execution context, and so its [call stack]() is simply a pile of references (i.e. conceptually stack-allocated). \*\**In recent updates, Python 3.11 splits the stack frame object into: a PyFrameObject and a struct [_PyInterpreterFrame](). Also, Python 3.12, as proposed in [PEP 709](https://peps.python.org/pep-0709/), inlines comprehensions into the code rather than compiling them as nested functions that isolate iteration variables*.\*\*
