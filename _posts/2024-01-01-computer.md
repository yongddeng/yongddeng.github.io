...
<!-- - <img src="https://www.skenz.it/lib/exe/fetch.php?tok=011de2&media=https%3A%2F%2Fwww.skenz.it%2Frepository%2Fcompilers%2Fllvm%2Fllvm.jpeg" width="500" alt="https://hyfshishen.github.io/tutorial-01-llvm.html">  -->
...
Triton is... There are many alternatives such as... Therefore, one use other backend besides LLVM compiler (i.e. LLC) for purposes.
...
An optimal compilation can accelerate model execution with CUDA/HIP graph...  Many deep learning compilers, such as [NVIDIA's CUDA Compiler]() (NVCC), use LLVM as its core. Many DL compilers tend to be graph-based... LLVM IR vs. Domain-Specific IR (TVM, Triton, TensorRT, CUDA, ..)... Triton is higher-level than CUDA... Optimization Focus: LLVM focuses on general code optimization for various hardware. CUDA and TensorRT emphasize GPU-specific optimizations, while Triton targets specific patterns of tensor computation for machine learning... A kernel fusion and quantisation (see below) are essential part of its LLVM-kind optimisation pipeline... Image source from [Li et al. (2020)](https://arxiv.org/abs/2002.03794)...
...
Note that AWS Inferentia was created with the single purpose of accelerating machine learning inference, unlike NVIDIA GPUs which were initially created to accelerate graphics. Each AWS Inferentia chip has 4 NeuronCores, and each NeuronCore is a systolic-array matrix-multiply engine with two stage memory hierarchy and a very large on-chip cache. A program is hardware dependent, so a model developed using Pytorch must be compiled into a specific format. Just like NVIDIA’s TensorRT compiler for GPUs, AWS Neuron SDK and Compiler supports optimization for efficient inference.
...
More detailed explanations for begineers... 

- <iframe width="550" height="300" src="https://www.youtube.com/embed/m8G_S5LwlTo?si=oVZN98jpLrCEqIju" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
