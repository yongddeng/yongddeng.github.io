...
[Low-level virtual machine]() (LLVM) is an open source toolkit for building and optimising compilers, converting source code into machine code that can be read and executed by a processor. It uses a language-independent [intermediate representation]() (LLVM IR), that is in either a portable, human-readable text format (.ll) or an assembly-like bitcode (.bc), both of which can be converted into one another. Given that a native compiler of high-level languages does all the heavy lifitng, a set of .bc files with optimised IR can be linked and/or compiled into machine code using the static compiler (i.e. *llc* cmd) or executed directly using the jit-compile interpreter (i.e. *lli* cmd).
...
[Compute unified device architecture]() (CUDA) is a general-ourpose GPU programming framework, which acts as a backend compiler and runtime for executing code on NVIDIA GPUs... NVCC, a LLVM-based domain specific compiler under the hood, translates CUDA code (in C, C++, or Fortran) into GPU executable code...  Specifically, i) parallel thread execution (PTX): An interim representation; and ii) machine code (CUBIN): GPU-specific binary code; The compiler passes CPU-specific portions of the CUDA code to a standard compiler like GCC or Clang, while GPU-specific portions (marked by *device* or *global*) are passed into PTX or machine code;

<!-- https://tinkerd.net/blog/machine-learning/cuda-basics/ -->

An optimal compilation can accelerate model execution with CUDA/HIP graph...  Many deep learning compilers, such as [NVIDIA's CUDA Compiler]() (NVCC), use LLVM as its core. Many DL compilers tend to be graph-based... There are many alternatives such as... Triton built by OpenAI is an open source that is more abstract than CUDA... 
