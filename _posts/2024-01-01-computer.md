...
Facebook utilised LibTorch and added *torch.jit.trace* to support JIT compilation. It records operations during a forward pass with fixed inputs, and emits a SCG that is ideal for models with predictable execution paths (i.e. control flows). They later added *@torch.jit.script*, which directly compiles Python code into [TorchScript]() IR, to support input-dependent (i.e. dynamic) models. Note that any TorchScript program can be saved from Python and loaded in a non-Python environment (e.g. train.py → deploy.cpp). In addition, a dynamic model in PyTorch can also be exported to the [Open Neural Network Exchange]() (ONNX) if it complies with ONNX's static graph requirements.
