...
[Compute unified device architecture]() (CUDA) is a general-ourpose GPU programming framework, which acts as a compiler and runtime for executing code on NVIDIA GPUs. NVCC, a LLVM-based domain specific compiler, translates CUDA code (in C, C++, or Fortran) into GPU executable code..  Specifically, i) parallel thread execution (PTX): An interim representation; and ii) machine code (CUBIN): GPU-specific binary code; The compiler passes CPU-specific portions of the CUDA code to a standard compiler like GCC or Clang, while GPU-specific portions (marked by *device* or *global*) are passed into PTX or machine code;
...
An optimal compilation can accelerate model execution with CUDA/HIP graph...  Many deep learning compilers, such as [NVIDIA's CUDA Compiler]() (NVCC), use LLVM as its core. Many DL compilers tend to be graph-based... A kernel fusion and quantisation (see below) are essential part of its LLVM-kind optimisation pipeline... Image source from [Li et al. (2020)](https://arxiv.org/abs/2002.03794)...

- LLVM IR vs. Domain-Specific IR (TVM, Triton, TensorRT, CUDA, ..)
- Triton is higher-level than CUDA
- Optimization Focus: LLVM focuses on general code optimization for various hardware. CUDA and TensorRT emphasize GPU-specific optimizations, while Triton targets specific patterns of tensor computation for machine learning
