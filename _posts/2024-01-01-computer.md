...
The [word length]() of the CPU represents the number of [bits]() (e.g. 32-bit, 64-bit) it can process in a single operation, typically deciding the size of registers, data bus width, and [instruction set architecture]() (ISA). A larger word length allows it to process more data at once or address larger memory spaces. Using types that match the CPU's native word length can often result in efficient computations, but languages including C define the [data type]() size only by minimum standards (e.g. *char*: 1 byte, *int*: 4 btyes) to ensure portability across CPU architectures. Modern CPUs usually use techniques such as [pipelining]() and [branch prediction]() to maximise instruction throughput.
