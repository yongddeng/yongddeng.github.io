...
They also featured [TorchScript]() (TS) based on LibTorch to enable [script mode](). The two front-end compilation APIs: [*torch.jit.trace*]() and [*torch.jit.script*]() can produce a SGD as an TS IR. The former runs a model with [proxies]() and only traces (i.e. records) tensor operations, whereas the latter parses the entire source code in .py. Scripting can be more flexible as it analyses pure Python code (e.g. dynamic control flows), and TS compiled programs can be executed by the PT JIT runtime or exported into non-Python env. (e.g. train.py $\to$ deploy.cpp), where the [Open Neural Network Exchange]() (ONNX) is the industry standard format that they may need to comply with.
