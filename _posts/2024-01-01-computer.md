...
A front-end compiler generates an IR through i) lexical analyser (Lexer): breaks the [source code]() into tokens (e.g. identifier, literal, and keyword); ii) parser: builds an [abstract syntax tree]() (AST) on the structure of the tokens and the language's grammar to represent the hierarchical structure of the source code; iii) semantic analyser: ensures correctness of the program beyond syntax (e.g. type checking, variable declarations, and scope resolution); and iv) IR generator: transforms the validated source code into a universal IR; Modern C compilers (e.g. GCC, and Clang) are [self-hosting]() (i.e. written in C/C++) and were developed via a [bootstrapping]() process.

Subsequently, the [LLVM Core]() takes responsibilities in IR optimisations (e.g. dead code elimination), and a back-end can either compile the outputted IR [ahead-of-time]() (AOT) (i.e. *llc* cmd), or execute it directly using a [just-in-time]() (JIT) compilation interpreter (i.e. *lli* cmd). The expressive power of the IRs and their ability to understand the high-level semantics determine sophistications of a compiler. That is, most high-level languages developed their own AST for associated infrastructures (e.g. Swift-SIL, Rust-MIR, Pytorch-TorchScript), and therefore, resulted various modules (i.e. for similar tasks) to unnecessarily co-exist as in language- and/or domain-specific IRs.
...
CUDA programming in C/C++ is however considered to be tough because i) memory coalescing; ii) SMEM synchronizstion and conflict management; iii) scheduling within SMs; and iv) scheduling across SMs; must all be coded manually. To simplify the process of writing optimised GPU code for ML researchers/practitioners, OpenAI released [Triton]() in 2021, utilising the LLVM-MLIR infrasturcture for a Python-based programming framework and DSL. Triton automates the first three challenges that are naturally inherent to the SIM"T" model, by enabling tile-based programming with multi-dimensional blocks (i.e. rather than individual "T"hreads).
