...
Fb utilised LibTorch to support JIT compilation via *torch.jit.trace*. It constructs a SCG from (tensor-) operations during a forward pass with fixed inputs, capturing models with predictable execution paths (i.e. no control flows). Subsequently, they added  *@torch.jit.script*, which directly compiles Python code into [TorchScript]() IR, to support input-dependent (i.e. dynamic) models. Note that any TorchScript program can be saved from Python and loaded in a non-Python environment (e.g. train.py → deploy.cpp). A dynamic model in PyTorch could also be to exported to the [Open Neural Network Exchange]() (ONNX) if they comply with ONNX's static graph requirements.
