...
Cross compiling became easier by LLVM but, despite tons of research and know-how, compilers cannot be easily developed or widely used. Accordingly, interpreters gained widespread adoption among people who prefer iterative and experimental workflows. Modern interpreters compile source code into [bytecode]() that is i) hardware-agnostic: it does not rely on the CPU's [instruction set architecture]() (ISA); and ii) executable on any platform where a compatible [virtual machine]() (VM) is ready (e.g. Linux, macOS, or embedded systems). This compilation is often faster (i.e. than -to binary) and interpreters execute instructions sequentially (i.e. line-by-line) to be  dynamic.

For instance, [CPython]() (i.e. Python) compiler converts source code (i.e. .py) into bytecode (i.e. .pyc), then the stack-based interpreter, [Python VM]() (PVM), executes bytecode during runtime by interacting with the OS and underlying hardware.... [PEP 339](https://peps.python.org/pep-0339/) describes the design of the CPython compiler.... The *dis* module can inspect bytecode, revealing the IR generated from source code (i.e. string or script).... 
...
[PEP 3147](https://peps.python.org/pep-3147/) introduced .pyc repository directories, called [\_\_pycache\_\_](), to better align clutter and enhance cross-version compatibility. They ....
