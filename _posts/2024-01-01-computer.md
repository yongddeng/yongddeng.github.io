...
[TorchDynamo]() (i.e. *torch._dynamo*) is the tracer used in [*torch.compile*](), leveraging the [FX]() (i.e. Python-to-Python transformation toolkit) to produce clean graphs that are compatible with downstream compilers. During graph acquisition, TorchDynamo [hooks]() into Python's bytecode execution via CPython's [Frame Evaluation API](). It then calls *torch.fx.symbolic_trace* to simulate operations using [proxies]() and identify capturable regions within a program, that are recoded as nodes of an [FX graph](). Dynamo breaks a computation graph when it encounters unsupported Python features (e.g. data-dependent control flow), and thus delegate it to CPython (i.e. default interpreter).
  
Apparently, *torch.compile* has the *fullgraph (bool)* parameter. With its default backend (i.e. Inductor), if the argument is *False (default)*, Dynamo searches compileable regions in the function. If set to *True*, the entire function must be capturable as a single graph, while an error is raised whenever it fails (i.e. graph breaks). In fact, the components of *torch.compile* can be used independently. For example, symbolic tracing may be invoked in isolation to capture a form of the code only for analysis purposes (i.e. not transformation), or code generator which produces an FX graph may be used for programmatically generating models (e.g. from an arbitrary configuration file).

<!-- *torch._dynamo.explain* and *TORCH_LOGS=graph_breaks* support debugging -->
