...
The LibTorch was further used to enable [script mode]() via two main APIs. *torch.jit.trace* builds a SCG using tensor operations executed on fixed inputs during a forward pass, capturing models only with predictable paths (i.e. fixed control flows). Beside the tracer, they featured *@torch.jit.script*, compiling input-dependent models directly from Python code into [TorchScript]() IR. Notably, any TorchScript programs can be saved from Python and loaded into non-Python environments (e.g. train.py $\to$ deploy.cpp). Dynamic models could be exported to the [Open Neural Network Exchange]() (ONNX) format, though they must comply with ONNX's static graph requirements.
