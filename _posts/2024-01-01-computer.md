...
A computer memory is essentially an array of bytes each with a unique address. That is, [random access memory]() (RAM), register, and cache are all some abstractions. The latency of  [dynamic RAM]() (DRAM) can be estimated as $T_{\text{precharge}} + T_{\text{row activation}} + T_{\text{col. access}}$. The access is fastest when data is in an active row (i.e. as contiguous) given that the physical design of memory cells also matters. [Static RAM]() (SRAM) is better suited for cache memory due to its high speed and stability, despite its higher cost. A disk sometimes can be used as [virtual memory](), alongside [paging]() or [segmentation](), to free up RAM for a process, while excessive swapping yields [thrashing]().
...
[Kwon et al. (2023)](https://arxiv.org/abs/2309.06180) open-sourced [vLLM](https://arxiv.org/abs/2309.06180) based on two main ideas...

- Batching is a way of utilising parallel computing during inference... Longer sentences in a batch monopolises the GPU resource while other sentences in the same batch are fully answered and queuing to be returned... 
  - [Continuous batching]() dynamically batch requests, updating batches while the request is still running. It achieves this by leveraging how LLMs perform inference: a looping process of repeatedly generating the next token in their response (i.e. once per output word)...  It can add requests to the batch on the fly and return early results when one record from a batch is completely done...
  
- Existing systems store KV pairs in contiguous memory spaces, limiting memory sharing and leading to inefficient memory management. The authors emphasised the inefficiency of managing KV cache memory in current LLM serving systems (e.g. FasterTransformer and Orca). 
  - [PagedAttention](): an attention algorithm inspired by virtual memory and paging (i.e. copy-on-write) techniques commonly used in operating systems. It enables efficient memory management by allowing for non-contiguous storage of attention keys and values. Leveraging techniques like virtual memory and copy-on-write, vLLM efficiently manages the KV cache and handles various decoding algorithms
  - It allows storing continuous KV pairs in non-contiguous memory space by partitioning the KV cache of each sequence into KV block tables. This way, it enables the flexible management of KV vectors across layers and attention heads within a layer in separate block tables, thus optimizing memory usage, reducing fragmentation, and minimizing redundant duplication
