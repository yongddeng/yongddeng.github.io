...
The [CPython]() compiler's design is outlined in [PEP 339](https://peps.python.org/pep-0339/), but in essence, its role is to transform source code (i.e. .py) into bytecode (i.e. .pyc), which consists of [operation codes]() (opcodes) and their corresponding [operands]() (i.e. if exists). Given that opcodes are low-level instructions, operands represent various entities depending on the context of opcode, which includes literal constants or immutables (e.g. number, string, tuple), variable or attribute names, or code blocks (e.g. function, class, comprehension). Each Python mnemonic corresponds to an integer of opcode, highly similar to how assembly instructions like *pop*, *push*, and *ret* are represented numerically.
