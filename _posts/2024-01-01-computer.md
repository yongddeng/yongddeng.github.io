...
Note that AWS Inferentia was created with the single purpose of accelerating machine learning inference, unlike NVIDIA GPUs which were initially created to accelerate graphics. Each AWS Inferentia chip has 4 NeuronCores, and each NeuronCore is a systolic-array matrix-multiply engine with two stage memory hierarchy and a very large on-chip cache. It supports FP16, BF16 and INT8 data types and doesn’t support higher precision formats because you don’t need it for inference — it is after all a specialized processor. Just like NVIDIA’s TensorRT compiler for GPUs, AWS Neuron SDK and Compiler supports optimization for efficient inference.
...
A program is hardware dependent, and [low-level virtual machine]() (LLVM) is an open source toolkit for building and optimising compilers, converting source code into machine code that can be read and executed by a processor. It uses a language-independent [intermediate representation]() (LLVM IR), which can be in either a portable, human-readable text format (.ll) or a more compact assembly-like bitcode (.bc). A .bc file can be compiled into machine code with the *llc* command or executed directly with *lli* (i.e. JIT-compile as an interpreter). Note that a native compiler of frontend languages does all the heavy lifitngs, while one can use other backend compiler other than LLC.
...
Triton is... There are many alternatives such as...
