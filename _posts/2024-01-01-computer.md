...
<!-- 
<img src="https://files.speakerdeck.com/presentations/4a236a815fc7455baf1277263bd1371f/slide_0.jpg" width="500" alt="https://speakerdeck.com/keitaw/practical-guide-on-pytorch-inference-using-aws-inferentia"> -->

### **Model Compilation**
<p style="margin-bottom: 12px;"> </p>

A program is hardware dependent... [Low-Level Virtual Machine]() (LLVM) is an open source compiler architecture. It can be the i) optimiser (i.e. language-agnostic optimisation); and ii) backend (i.e. machine code generation); of multiple programming languages. [LLVM IR]() is a low-level intermediate representation used by LLVM, allowing multiple frontends and backends (e.g. bytecode and machine code) to interoperate freely on the LLVM toolchain. E.g. Source code and Frontend compiler pairs: {C: Clang, C++: Clang, Go: Gollvm, Rust: rustc, ...} --> generate machine code (x86, ARM, RISC-V, ...) being compatiable to the hardware.

<!-- This process is also called “lowering”, as in you “lower” your high-level framework code into low-level hardware-native code. It’s not “translat?ing” because there’s no one-to-one mapping between them... -->
...
From the original code for your models, compilers generate a series of high- and low-level interim representations before generating hardware-native code to run your models on a certain platform... That is, an optimal compilation can accelerate model execution with CUDA/HIP graph...  Many uses LLVM as its core and [NVIDIA's CUDA Compiler]() (NVCC) is also based on LLVM, allowing developers to create or extend programming languages (i.e. via the NVIDIA Compiler SDK) with support for GPU acceleration... Note that a kernel fusion (see below) is one part of its optimisation pipeline... Image source: [Li et al. (2020)](https://arxiv.org/abs/2002.03794)...
...
CUDA is a programming framework and platform, with dedicated NVCC (i.e. LLVM-based custom compiler) for translating CUDA code into executable GPU programs. CUDA provides a set of APIs, libraries, and tools for developers to write programs for NVIDIA GPUs. NVCC lowers high-level CUDA code (in C, C++, or Fortran) into i) parallel thread execution (PTX): An interim representation; and ii) machine code (CUBIN): GPU-specific binary code; The compiler passes CPU-specific portions of the CUDA code to a standard compiler like GCC or Clang, while GPU-specific portions (marked by *device* or *global*) are passed into PTX or machine code;
