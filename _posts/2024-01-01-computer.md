...
[Kwon et al. (2023)](https://arxiv.org/abs/2309.06180) open-sourced LLM serving frameworks, called [vLLM](https://arxiv.org/abs/2309.06180), alongside two main ideas: i) It employs [continuous batching](), dynamically updating request batches during inference, allowing for more efficient GPU resource usage and faster early results; ii) the [PagedAttention]() algorithm, inspired by virtual memory and paging techniques commonly used in OS, enables efficient non-contiguous storage of $K$ and $V$, optimising memory usage and reducing fragmentation; vLLM also supports quantisation, along with optimised CUDA kernels for GPU performance, streamlining KV cache management and memory access. 
