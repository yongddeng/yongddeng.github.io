...
[TorchDynamo]() (i.e. Dynamo) serves as the front-end, leveraging PEP 523 (i.e. frame evaluation API) and the [functional transformation]() (FX) framework to represent Python code as high-level SCGs compatible with downstream compilers. For graph acquisition, Dynamo i) [hooks]() into bytecode execution using PEP 523; ii) analyses bytecode and identifies torch.\* and non-torch.\* bits of a program; and iii) convert compilable regions into a graph; *torch.fx.symbolic_trace* can be used with [proxies]() to simulate captured torch.\* operations and record them as nodes in an [FX graph](), while general Python code segment in non-torch.* fall back to CPython (i.e. default interpreter).
...
[Graph breaks]() can occur when Dynamo encounters complex behaviours. For example, [dynamic control flow]() refers to situations where the path of execution (which operation to apply) depends on runtime data. If one has *condition = lambda x: x.mean() > 0.5* that relies on the values held in *x: torch.Tensor*, then *if condition(x): return x * 2; else: return x / 2;* are broken down into two seperate FX graphs, leading Dynamo to trace the computation for each case (or, revert back to the interpreter). Frequent graph breaks mean [performance hit](), and APIs such as *torch._dynamo.explain* and *torch.fx.graph_module.GraphModule.print_tabular* are helpful for diagnosing graphs. <!-- Dynamo will revert back to  -->

- <div style="position: relative; display: inline-block;"> <img src="../assets/blog/2024-01-01-torchdynamo.png" width="500"> </div>
  
Besides, Dynamo inserts a set of [guards]() (as checkpoint markers) into the code to collectively validate if the runtime conditions remain consistent with the assumptions made during tracing FX graphs.... For example, if a tensor shape is shifted by a new batch size, the shape guard triggers a recompilation, discarding the cached graph and retracing the function to accommodate the new state. Dynamo supports [symbolic shape](), i.e. it analyses and generalises its expected shape whenever the shape guard fails.... 

<!-- press "cmd + ." with some lines of code-->
- <div style="position: relative; display: inline-block;background-color: darkgrey;"> <img src="https://depyf.readthedocs.io/en/latest/_images/dynamo-workflow.svg" width="500"> <a href="https://pytorch.org/get-started/pytorch-2.0/" target="_blank" style="position: absolute; bottom: -8px; right: 4px; font-size: 12px;">[src]</a> </div>

Complex PyTorch operations (e.g. *torch.nn.functional.linear*) in a graph can be decomposed into [PrimTorch]() primitives or ATen modules for better traceability and downstream optimisation, but Dynamo alone cannot generate backward graphs.... [AOTAutograd]() specialises in handling the complexities of PyTorch’s autograd system. It builds on Dynamo’s capabilities to generate efficient backward graphs in addition to the forward graph.... If your program does not require gradients (e.g. inference), AOTAutograd is not invoked.


FX graphs, as the container of key operations (i.e. computations) in a program, are core elements of Torch IR. Using Aten and PrimTorch, Torch IR can be converted to even lower-level representations and get closer to the target backend. PrimTorch are a set of highly optimized and hardware-specific operations that can further speed up the computation, particularly on GPUs. They are often used to replace more general operations with more specialized ones that take better advantage of hardware features.
...
<!-- Once Dynamo identifies subgraphs, AOTAutograd takes it and splits it into forward and backward (static-) computation graphs. -->

By treating the forward and backward passes as distinct subgraphs, AOTAutograd can optimize the forward pass independently for further optimisations (e.g. operation fusion, redundant computation elimination). It can also generate a minimal backward pass (which is often more computationally expensive) tailored to the forward pass, as performance is tightly coupled to the efficiency of the forward pass.... Ahead-of-time backward pass (i.e. a SCG construction) is in contrast to the PT 1.x's approach where the backward pass is dynamically generated during runtime.



<!-- AOTAutograd can employ techniques like activation checkpointing or recompute-on-demand to manage this memory-pressure tradeoff. -->

<!-- along with the prepackaged compilers -->

- <div style="position: relative; display: inline-block;"> <img src="https://global.discourse-cdn.com/flex036/uploads/pytorch1/optimized/1X/0acfd47dba84bd41567e518cdb6b51e5c5001444_2_870x1000.jpeg" width="500" height="540"> <a href="https://dev-discuss.pytorch.org/t/torchdynamo-update-6-training-support-with-aotautograd/570" target="_blank" style="position: absolute; top: 0px; left: 4px; font-size: 12px;">[src]</a> </div>
