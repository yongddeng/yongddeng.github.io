...
[TorchDynamo]() (i.e. Dynamo) serves as the front-end, leveraging PEP 523 (i.e. frame evaluation API) and the [functional transformation]() (FX) framework to represent Python code as high-level DCGs compatible with downstream compilers. For graph acquisition, Dynamo i) [hooks]() into bytecode execution using PEP 523; ii) analyses bytecode and identifies torch.* and non-torch.* bits of a program; and iii) convert compilable regions into a graph; Dynamo uses [*torch.fx.symbolic_trace*]() to simulate torch.* bits with [proxies]() and records tensor operations as nodes in an [FX graph](), while a [graph break]() occurs when it encounters ops. it cannot capture during dynamic tracing.

Apparently, *torch.compile* currently has a parameter [*fullgraph (boolean)*](), controlling the tracing behaviour. If the argument is set to *False (default)*, it identifies and compiles capturable regions in the function. If *True*, the entire function must be compiled as a single graph....
