...
Similar to CPUs, a [streaming multiprocessor]() (SM) serves as a core processing unit of the GPU responsible for thread block executions. When a [thread]() is the smallest unit of execution that handles a portion of tensor operation (e.g. element-wise mul.), an [warp]() is threads organised in a uniform size following the SIMT model, and a [block]() is a collection of warps assigned to each SM. If more thread blocks than the number of SMs can be properly dispatched in modern GPUs, then [warp schedulers]() will facilitate the switching between warps within the SMs whenever a core stalls, and enables maximum utilisation via effective latency masking and higher throughput.
