...
The [Python virtual machine]() (PVM), a stack-based interpreter, is responsible for executing bytecode at runtime by simulating a CPU's execution model (i.e. stack machine). In particular, the (legacy API-) *PyFrame_New* in Objects/frameobject.c creates a [*PyFrameObject*]() that models a [stack frame]() by holding the state of the executing code (e.g. references to the code, local and global namespaces, and stack-related information). A frame object has i) *[f_code]()*: a reference to the *PyCodeObject*; and ii) *[f_back]()*: a link to the last frame;, both of which are used to form a [linked list]() of *PyFrameObject* pointers. The Python built-in *inspect* module helps examining the two objects.
