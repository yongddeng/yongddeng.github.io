...
[TorchDynamo]() (i.e. Dynamo) serves as the front-end, leveraging PEP 523 (i.e. frame evaluation API) and the [functional transformation]() (FX) framework to represent Python code as high-level DCGs compatible with downstream compilers. For graph acquisition, Dynamo i) [hooks]() into bytecode execution using PEP 523; ii) analyses bytecode and identifies torch.* and non-torch.* bits of a program; and iii) convert compilable regions into a graph; *torch.fx.symbolic_trace* can be used with [proxies]() to simulate captured torch.* operations and record them as nodes in an [FX graph](). A [graph break]() occurs when it encounters operations it cannot capture during dynamic tracing.

Reducing the number of graph breaks is important. *torch._dynamo.explain* and *torch.fx.graph_module.GraphModule.print_tabular* are useful for debugging graph structures and breaks. One can also wrap function calls in a try-except block to enforce strict behavior and catch graph break exceptions early, aiding in the identification of performance bottlenecks at the Python-level. Apparently, *torch.compile* currently has a parameter [*fullgraph (boolean)*](), controlling the tracing behaviour. If the argument is set to *False (default)*, it identifies and compiles capturable regions in the function. If *True*, the entire function must be compiled as a single graph....
