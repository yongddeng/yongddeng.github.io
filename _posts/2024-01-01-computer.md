...
TS does not support some libraries or Python features (e.g. generator, comprehension), but rather forces incompatible modules to be either wrapped/avoided. TS-compiled models also loss some of eager charm (i.e. PT's eager execution flexibility). Thus, [*torch.compile*]() API is introduced as a domain-specific compiler of [Pytorch 2.0]() (PT2). It transforms PyTorch-related regions into a SCG (as TS does), while it continue to interpret general Python code as usual, maintaining its "pythonic" nature. They reported this dynamic trace (unlike TS-"once and for all") works 93% of the 163 open-source models, and a partially-compiled model runs 43% faster in training on A100.
