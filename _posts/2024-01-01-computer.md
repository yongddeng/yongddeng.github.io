...
[Low-level virtual machine]() (LLVM) is an open source toolkit for building and optimising compilers, converting source code into machine code that can be read and executed by a processor. It uses a language-independent [intermediate representation]() (LLVM IR), which can be in either a portable, human-readable text format (.ll) or an assembly-like bitcode (.bc), and can be easily converted into one another. Given that a native compiler of high-level languages does all the heavy lifitng, an optimised IR in .bc file can be compiled into machine code with the *llc* command or executed directly with *lli* (i.e. JIT-compile interpreter), and one may use other backend besides LLC.
