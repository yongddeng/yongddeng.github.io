...
The [Triton compiler]() captures the AST of a kernel, that is written in *triton.language* and also decorated by *@triton.jit*, and progressively generates IRs through MLIR dialects: i) Triton Tensor Graph IR (TTGIR): a hardware-agnostic IR for high-level opt.; ii) Triton-GPU IR: a hardware-specific IR optimised for efficient GPU code generation; and iii) LLVM IR: LLVM lowers any validated IRs into an executable; While it offers finer control over SMEM and HBM compared to eager mode PyTorch (i.e. has the C/C++ backend), the throughputs of the fused softmax in Trition seems higher than both the *F.softmax* and PyTorch-native implementation invoked with  *@torch.jit.script*.
