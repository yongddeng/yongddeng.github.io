...
[Graph breaks]() can occur when Dynamo encounters complex behaviours, such as [dynamic control flow](), which refers to situations where the path of execution depends on runtime data (e.g. new inputs). For example, if there exists *condition = lambda x: x.mean() > 0.5* that relies on the values held in *x: torch.Tensor*, then the code block *if condition(x): return x / 1.1; return x / y;* will be represented by two FX graphs which Dynamo must trace all to maximise compilability. Frequent graph breaks lead to [performance hit]() due to guards. APIs like *torch._dynamo.explain* and *torch.fx.graph_module.GraphModule.print_tabular* can be helpful for diagnosing and debugging breaks. <!-- Dynamo will revert back to  -->
