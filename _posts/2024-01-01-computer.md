...
In fact, TS does not support certain libraries or Python features (e.g. generator, comprehension), instead forces incompatible modules to be either wrapped or avoided. To overcome these limitations, the [*torch.compile*]() API was introduced as a domain-specific compiler in [Pytorch 2.0]() (PT2). This seperates PyTorch-related code in a program and represents them into a DCG for further optimisations and executions. Arbitrary Python code will be interpreted as usual, keeping the dynamic nature of the language. *torch.compile* conducts: i) graph acquisition (i.e. tracing); ii) graph lowering (i.e. high-level optimisation); and iii) seamless integration to prepackaged back-ends.
