...
### **3.3. CPython (Interpreter)**
<p style="margin-bottom: 12px;"> </p>

The emergence of Triton and the dominance of PyTorch (i.e. prior to 2.0) over TensorFlow might share a common root cause (i.e. an naive subjective interpretation). Compilers, despite decades of refinement—for example, GCC for C—remain challenging to use, particularly for ML researchers and practitioners who often prefer tools that align with the behavior of interpreters. This preference explains why [CPython]() (i.e., Python) gained widespread adoption: its dynamic nature, intuitive syntax, and extensive ecosystem of open-source libraries cater to the iterative and experimental workflows. Modern interpreters typically integrate a [virtual machine]() (VM) to facilitate JIT compilation. 

The Python compiler, which is a part of the CPython interpreter, converts "hot regions" (i.e. frequently executed code paths) of source code (i.e. .py) into a platform-independent [bytecode]() (i.e. .pyc). It is is then sequentially interpreted by the stack-based [Python VM]() (PVM), which operates at runtime by interacting with the operating system and underlying hardware. This process balances performance and responsiveness. [PEP 3147](https://peps.python.org/pep-3147/) reduced clutter and enhanced cross-version compatibility by introducing a dedicated directory for .pyc files, called \_\_pycache\_\_. The *dis* module allow one to inspect bytecode, revealing the IR generated from source code (e.g. a string or script). 

- <div style="position: relative; display: inline-block; background-color: white;"> <img src="https://i.sstatic.net/tv2Ir.png" width="500"> <a href="https://stackoverflow.com/questions/2998215/if-python-is-interpreted-what-are-pyc-files" target="_blank" style="position: absolute; top: 0px; left: 4px; font-size: 12px;">[src]</a> </div>

<!-- https://ayandas.me/assets/posts_res/7/process.jpg -->

Historically, interpreters represented source code as ASTs and executed them using [recursion](). However, these trees, while well-suited for program structure representation, were not optimised for execution. A range of optimisation was limited, and lowering source code  to machine code was also challenging. As a result, these limitations drove the development of JIT, which has since become a standard feature in modern interpreters. JIT compilers dynamically translate high-level code to machine code during runtime, enabling efficient executions, and finer optimisations such as register allocation, instruction scheduling, constant folding, and dead code elimination.
