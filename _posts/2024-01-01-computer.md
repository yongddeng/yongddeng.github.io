...
Pytorch 2.0 (PT2) is advertised with the *Torch.compile*  API.... It is the highest-level API that wraps around lower-level tools other than *torch._dynamo*.... Unlike general purpose compilers like gcc or llvm, torch.compile is a domain-specific compiler. Therefore, it only focuses on PyTorch related computation graph. Therefore, we need a tool to separate users code into two parts: plain python code and computation graph code. 

It aims to ...... PT2 prepackaged various backends.... i) training & inference: {TorchInductor, CUDA graphs, IPEX, ONNX}; ii)  inference-only: {Torch-TensorRT, IPEX , TVM, OpenVINO}. Note that IPEX is for CPU. By using user-defined Triton kernels with torch.compile, you can integrate these optimized computations into your PyTorch model, potentially achieving significant performance improvement. That is, a kernel fusion and quantisation (see **) are core part of its LLVM-kind optimisation pipeline.... A graceful compilation requires some understanding of its internals ....
...
<!-- CUDA is closed, thus we shall actively utilise Triton...
...
Compiled graphs are cached in the Python process's (in-) memory and reused to strike a balance between Python's dynamic nature and high execution speed of static graphs. Therefore, reducing the number of graph breaks is important. [*torch._dynamo.explain*]() and [*torch.fx.graph_module.GraphModule.print_tabular*]() are useful for debugging graph structures and breaks. One can also wrap function calls in a try-except block to enforce strict behavior and catch graph break exceptions early, aiding in the identification of performance bottlenecks at the Python-level. 
