...
[TorchDynamo]() (i.e. Dynamo) serves as the front-end, leveraging PEP 523 (i.e. frame evaluation API) and the [functional transformation]() (FX) framework during Python-level graph acquisition. Simply put, Dynamo i) hooks into bytecode execution via PEP 523; ii) analyses bytecode and captures torch.\* and non-torch.\* calls; and iii) convert compilable regions into a graph; For some proxies, one can use *torch.fx.symbolic_trace* (or, *torch._dynamo.export*) to see an [FX graph](), where ops. on a [TensorVariable]() adds a FX node in the graph. For example, a .py may contain {[torch.* ops, torch.tensor]: TorchVariable, Py\_builtin\_variables: BuiltInVariable, Py\_lists: ListVariable}.
