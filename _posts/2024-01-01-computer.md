...
Cross compiling became easier by LLVM but, despite tons of research and know-how, compilers cannot be easily developed or widely utilised. Accordingly, interpreters gained widespread adoption among users who favour iterative and experimental workflows. Modern interpreters compile source code into [bytecode]() that is i) hardware-agnostic: it does not rely on the CPU's [instruction set architecture]() (ISA); and therefore ii) executable on any platform where a compatible [virtual machine]() (VM) is available (e.g. Linux, macOS, Window). This compilation is often faster (i.e. than -to binary) and interpreters execute instructions sequentially (i.e. line-by-line) to be dynamic.

The [CPython]() compiler's design is outlined in [PEP 339](https://peps.python.org/pep-0339/), but in essence, its role is to transform source code (i.e. .py) into bytecode (i.e. .pyc), which consists of [operation codes]() (opcodes) and their corresponding [operands]() (i.e. if exists any). Given that opcodes are low-level instructions, operands represent various entities, such as literal constants or immutables (e.g. number, string, tuple), variable or attribute names, or code blocks (e.g. function, classe, comprehension), depending on the opcode's context. Therefore, each Python mnemonic is mapped to an integer of opcode, similar to how assembly instructions like *pop*, *push*, and *ret* have their own integers.

Bytecode, formed by combining opcodes and operands, serves as an IR that the CPython interpreter can efficiently execute. But still, CPython continually improves performance through various optimisations. For instance, the [adaptive interpreter]() shipped by [PEP 659](https://docs.python.org/3/whatsnew/3.11.html#whatsnew311-pep659) dynamically replaces generic opcodes with specialised, faster versions at runtime based on profiling data. It reduces overhead for frequently executed instructions, and thus  significantly enhances execution speed. One can of course disassemble Python opcodes using the *dis* module or examined further in the CPython source code: [[Include/opcode.h](https://github.com/python/cpython/blob/main/Include/opcode.h), [Python/opcode_targets.h](https://github.com/python/cpython/blob/main/Python/opcode_targets.h), [Lib/opcode.py](https://github.com/python/cpython/blob/main/Lib/opcode.py)]. 
...
During the intreptation phase, a struct [*PyCodeObject*]() in the C implemenation encapsulates the compiled bytecode and its associated metadata. This custom data type contains i) *co_code*: the sequence of opcodes and operands representing the bytecode instructions; ii) *co_consts*: constants (e.g. string, number, tuple) or nested *PyCodeObject* instances; Furthermore, it also has iii) *co_names*: variable and attribute names used by the bytecode; iv) *co_varnames*: local variable names; *co_freevars* / *co_cellvars*: variables used in closures; *co_filename* / *co_name*: the source file and code block name, respectively; <!-- So, it acts as a blueprint for a frame (e.g. function call) that has been pushed to a stack -->

The above design allows Python to manage complex code structures while preserving essential runtime information. Havign said that, the [Python virtual machine]() (PVM) is a stack-based bytecode interpreter (i.e. resembles a CPU in its design) which reads and executes each opcode stored in the *PyCodeObject* during runtime. Explicitly, it creates a struct called [*PyFrameObject*]() to model a stack frame using the attributes: i) a reference to the *PyCodeObject* being executed (*f_code*); ii) local and global variable dictionaries; iii) The current instruction pointer; iv) a pointer to the previous frame (*f_back*), forming a set of frames as a linked list; 

The function [*PyEval_EvalFrameEx*]() (i.e. renamed [*PyEval_EvalFrameDefault*]() in Python 3.9+), which locates in the cpython/Python/ceval.c directory, operates as the main interpreter loop, taking a *PyFrameObject* and executing Python bytecode in the context of the frame....

<!-- During execution, all the static information about the program store in *PyCodeObject*, and *PyFrameObject* is a simulation of stack frame in x86 platform, such that a set of PyFrameObject are organised like a single linked-list.... The C function that executes a Python (i.e. written in C) function is called, mellifluously, *PyEval_EvalFrameEx*. It takes a Python stack frame object and evaluates Python bytecode in the context of the frame. -->
