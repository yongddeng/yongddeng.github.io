...
[Alban Desmaison](https://pytorch.org/docs/stable/community/persons_of_interest.html) also commented: "Given that Torch7 (i.e. written in [Lua]()) relied on these C/C++ libraries: Tensor Handling (TH/THNN) and Tensor Handling CUDA (THC/THCUNN on CUDA dependencies) for CPU/GPU computing, i) the initial PyTorch is simply a Python wrapper + new autograd engine + TH*; ii) Aten was developed later to provide C++ interfaces, allowing an implementation of new PyTorch functionalities can be done using within ATen; iii) [LibTorch]() is the distribution of PyTorch, a replacement and/or supplmenet for the Python wrapper (which used [Pybind11]() to wrap ATen), enabling more direct access to the core backends through the C++ API".
...
The LibTorch was also used to support the JIT compilation. *torch.jit.trace* builds a SCG based on tensor operations executed on fixed inputs during a forward pass, capturing models only with predictable paths (i.e. fixed control flows). Fb extended the functionality via *@torch.jit.script*, directly compiling input-dependent models from Python code into [TorchScript]() IR. Notably, TorchScript programs can be saved from Python and loaded into non-Python environments (e.g. train.py $\to$ deploy.cpp). Dynamic models in PyTorch can also be exported to the [Open Neural Network Exchange]() (ONNX) format, though they mush comply with ONNX's static graph requirements.
