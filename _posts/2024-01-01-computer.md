...
GPU compute performance is measured in [floating-point operations per second]() (FLOPS). Peak FLOPS depends on core count, clock speed, and precision format, with lower precision yielding higher throughput since more operations fit per cycle. For instance, the A100 delivers 19.5 TFLOPS at FP32 but 312 TFLOPS at FP16 via tensor cores. Achieved FLOPS falls well below peak in practice due to memory bottlenecks, kernel launch overhead, and idle cycles. In addition, [Model FLOPS utilisation]() (MFU), the ratio of achieved to theoretical peak, is the standard efficiency metric for training runs, where 30-60% is typical at scale.

Note that FLOPS and FLOPs look almost identical but measure different things. FLOPS (uppercase) is a rate, counting floating-point operations completed per second, used in peak-performance claims like '312 TFLOPS on the A100.' FLOPs (lowercase) is a count, the total operations in a workload, used in ratios like arithmetic intensity (FLOPs/byte) or training budgets ($\sim 3.14 \times 10^{23}$ FLOPs for GPT-3). The two relate via "wallclock time" $\approx$ "total FLOPs" $/$ "effective FLOPS", so the same workload runs faster either by reducing its FLOPs (e.g. quantisation, sparsity) or by raising delivered FLOPS (e.g. better kernels, higher MFU).
