...
The compiler of CPython compiles source code into [bytecode]() (i.e. .pyc). A [CodeObject]() is generated during the compilation process to store the bytecode instructions along with static data (e.g. consts list, variable names, exception table) required for execution. As the *PyEval_EvalCode* function in [Python/ceval.c]() executes a CodeObject, a [PyFrameObject]() is constructed to hold the runtime state for the execution, where its *f_code* attribute points to the corresponding CodeObject (i.e. the CodeObject acts as a blueprint). By default, the frame is executed by *_PyEval_EvalFrameDefault*, but as per [PEP 523](), the evaluation function can be configured with a custom one.
