...
PyTorch (PT), evolved from the [Torch7]() library, is an open-source deep learning (DL) framework developed by Facebook. TensorFlow 1.x export [static computation graph]() (SCG) for efficient serialisation and deployment, but PT 1.x instead export [dynamic computation graph]() (DCG) for flexibility and easy debugging. [*torch.Tensor*]() class is the core data structure representing homogeneous, multi-dim. rectangular arrays. While its key attributes: [*'requires_grad', 'grad', 'grad_fn'*] enable [automatic differentiation]() (AD) within a DCG, a tensor also contains metadata which describe its shape: [*'size', 'stride', 'dtype', 'device', 'layout'*], similar to *numpy.array*.
