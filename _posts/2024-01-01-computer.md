...
Specialised processors offload computational workloads from the CPU, enhancing performance for specific tasks. GPUs are currently the most widely chosen, but [Application-specific integrated circuits]() (ASICs), while typically not programmable or developer-friendly, offer greater efficiency than GPUs for specific use cases. Notable ASIC-based hardware includes AWS Inferentia (Inf1) and Trainium, Google's TPU, and Apple's Neural Engine (ANE). For operations not supported by an ASIC, execution falls back to the CPU, but the most importantly, programs targeting a specialised accelerator are hardware-dependent and require compilation into executables tailored for the target machine. 

Specialised processors offload computational workloads from the CPU, enhancing performance for specific tasks. GPUs are the most widely recognised, but [application-specific integrated circuits]() (ASICs), while less programmable and developer-friendly, offer greater efficiency for specific use cases. Notable ASIC-based hardware includes AWS Inferentia (Inf1), Trainium, Google's TPU, and Apple's Neural Engine (ANE). Programs which rely on specialised accelerators are hardware-dependent and require compilation for the target machinery, while unsupported operations (e.g. custom activation functions in a ML model) will be executed on the CPU as a [fallback](https://www.quora.com/What-is-CPU-fallback).
...
The LibTorch was also used to support the JIT compilation. *torch.jit.trace* builds a SCG based on tensor operations executed on fixed inputs during a forward pass, capturing models only with predictable paths (i.e. fixed control flows). Fb extended this functionality via *@torch.jit.script*, compiling input-dependent models directly from Python code into [TorchScript]() IR. Notably, TorchScript programs can be saved from Python and loaded into non-Python environments (e.g. train.py $\to$ deploy.cpp). Dynamic models in PyTorch could also be exported to the [Open Neural Network Exchange]() (ONNX) format, though they must comply with ONNX's static graph requirements.
