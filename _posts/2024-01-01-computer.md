...
[\_\_torch_dispatch\_\_]() does .... \_\_torch_dispatch\_\_ is the only way to go from C++ back into Python....  At the end of \_\_torch_dispatch\_\_ tracing, AOT Autograd has a forward graph and joint forward-backward graph. AOT Autograd then uses a partitioner to isolate the forward and backward graph.... AOT may recompute (i.e. [activation checkpointing]()) some activations during the backwards pass to saves memory (i.e. but we incur performance overhead). However, in the presence of fusing compiler, one can recompute the fusion-friendly operators, and then rely on the fusing compiler to fuse the recomputed operators. This reduces both memory and runtime....
...
AOT, in conjunction with Dynamo, is what actually brings performance improvements by tracing both the FWD and BWD passes as a joint graph and partitioning it into two separate FXs graphs. In particular, it leverages [PrimTorch]() (torch/_prims), a minimal abstraction of ATen tailored for graph-based workflows, which consolidates PyTorch's ~2,000 operators into a streamlined set of ~250 primitives. The pointwise operators are memory bound, but the primitives can map to low-level hardware instructions, and so appropriate back-end compilers can fuse operations: *{"NNC": "fuse pointwise ops. for CPU devices", "nvFuser": "an alternative for CUDA devices"}*.
