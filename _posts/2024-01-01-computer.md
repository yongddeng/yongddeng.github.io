...
[TorchDynamo]() (i.e. Dynamo) serves as the front-end, leveraging PEP 523 (i.e. frame evaluation API) and the [functional transformation]() (FX) framework to represent Python code as high-level DCGs compatible with downstream compilers. For graph acquisition, Dynamo [hooks]() into bytecode execution via PEP 523 and uses [*torch.fx.symbolic_trace*]() to simulate code on [proxies](), and so it dynamic identifies compilable regions within a prorgram and records tensor operations as nodes in an [FX graph](). Operations that Dynamo cannot support (e.g. *print* tensors, tensor-dependent if-statements) fall back to CPython (i.e. default interpreter). 
<!-- 

[TorchDynamo]() (i.e. Dynamo) is the tracer of *torch.compile*, which leverages the [functional transformation]() (FX) framework to transform Python code into high-level SCGs compatible with downstream compilers. A graph acquisition beigns with a bytecode execution [hook]() via the CPython's [frame evaluation API]() (PEP 523). Dynamo then uses [*torch.fx.symbolic_trace*]() to internally simulate code for the given [proxies](), allowing it to dynamically identifying compilable regions of a prorgram and record tensor operations as nodes in an [FX graph](). Operations of unsupported features (e.g. *print* tensors, tensor-dependent if-statements) fall back to CPython (i.e. default interpreter).  -->
...
However, a [graph break]() occurs when Dynamo cannot fully trace a segment of code into the FX graph. It is forced to split the program into two parts: i) FX graph captures tensor operations that can be statically compiled; ii) arbitrary Python code that remains under the control of the interpreter; *torch.compile* currently has a parameter [*fullgraph (boolean)*](), controlling the tracing behaviour. If the argument is set to *False (default)*, it identifies and compiles capturable regions in the function. If *True*, the entire function must be compiled as a single graph....


Reducing the number of graph breaks is important. [*torch._dynamo.explain*]() and [*torch.fx.graph_module.GraphModule.print_tabular*]() are useful for debugging graph structures and breaks. One can also wrap function calls in a try-except block to enforce strict behavior and catch graph break exceptions early, aiding in the identification of performance bottlenecks at the Python-level. 
