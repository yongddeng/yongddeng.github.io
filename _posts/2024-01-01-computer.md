...
The compiler of CPython compiles source code into [bytecode]() (i.e. .pyc). A [CodeObject]() is generated during the compilation process to store the bytecode instructions along with static data (e.g. consts list, variable names, exception table) required for execution. As the *PyEval_EvalCode* function in [Python/ceval.c]() executes a CodeObject, a [PyFrameObject]() is constructed to hold the runtime state for the execution, where the *f_code* attribute points to the corresponding PyCodeObject (i.e. the CodeObject acts as a blueprint). This frame is finally executed by *_PyEval_EvalFrameDefault*. As per [PEP 523]() the evaluation of frames is customisable with \[interp $\to$ eval_frame\].
