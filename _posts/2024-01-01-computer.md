...
Its memory hierarchies are also optimised for high throughput and low latency. For instance, the V100 has integrated [shared memory]() (SMEM) and L1 cache, allowing L1 cache to enjoy SMEM’s high-bandwidth performance and low-latency. Both SMEM and L1 cache will store more frequently accessed data to reduce the need for slower global memory access (i.e. VRAM). Whereas,  L2 cache, an off-chip memory shared across all SMs, enhances throughput to L1/SMEM by enabling quick access to larger datasets. This facilitates efficient data flow between SMs and the [high-bandwidth memory]() (HBM). 

A high performance of HBM is essentially attributed to its vertical stacking architecturem, where multiple DRAM dies are layered on top of each other and interconnected by [through-silicon vias]() (TSVs). In the meantime, an NVIDIA's [architect](https://www.youtube.com/watch?v=n6M8R8-PlnE&t=75s) mentioned as did industry, the primary bottleneck in GPU utilisation is memory bandwidth, not peak TFLOPS. More specifically, the highest bandwidth of the A100 is theoretically 9,750 GB/s (64 bytes/SM × 108 SMs × 1410 MHz), yet HBM2's 1,555 GB/s is 6.3 times less than what the SMs require. CUDA developers should be aware of the contiguous memory allocation as CUDA (i.e. C for GPU) does not enforce it.
