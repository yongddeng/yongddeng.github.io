...
Dynamo inserts a set of [guards]() (i.e. checkpoint markers) into the code to efficiently and collectively validate if the runtime conditions remain consistent with the assumptions made at JIT compilation (i.e. FX graphs construction). For instance, the shape guard triggers a recompilation when a tensor shape is shifted by a new batch size. It discards the cached graph and retraces the function to take in the new state....
