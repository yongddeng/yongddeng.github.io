...
Pytorch 2.0 (PT2) is advertised with the *Torch.compile*  API.... It is the highest-level API that wraps around lower-level tools other than *torch._dynamo*.... 
...
- <div style="position: relative; display: inline-block;"> <img src="https://pytorch.org/assets/images/pytorch-2.0-img12.png" width="500"> <a href="https://pytorch.org/get-started/pytorch-2.0/" target="_blank" style="position: absolute; top: 0px; left: 4px; font-size: 12px;">[src]</a> </div>
...
[TorchDynamo]() (i.e. *torch._dynamo*) is the tracer used in *torch.compile*, leveraging the [functional transformation]() (FX) toolkit to convert Python code into high-level computation graphs compatible with downstream compilers. The graph acquisition [hooks]() (i.e. intercepts) Python bytecode execution via CPython's frame evaluation API, and uses [*torch.fx.symbolic_trace*]() to simulate operations with [proxies](), allowing it to record operations as nodes in an [FX graph](). It dynamically identifies capturable regions of a prorgram while neatly falling back to the CPython interpreter for operations involving unsupported features, such as *print* function or data-dependent control flow.

Simultaneously, diverse types of [guards]() (e.g. tensor shape, control flow, device config., litral) collectively ensure both the correctness and efficiency of compiled graphs by validating that runtime conditions remain consistent with the assumptions made during tracing. For instance, if a tensor shape is shifted according to a new batch size, the corresponding guard triggers a recompilation, discarding the cached graph and retracing the function to accommodate the new state. Caching graphs in the Python process's memory (i.e. in-memory) and using them strikes a balance between Python's dynamic nature and the performance benefits of static graph execution.
...
<!-- The export mode allows .... The eager mode is simply .... -->

Reducing the number of graph breaks is important for improving performance. [*torch.fx.graph_module.GraphModule.print_tabular*]() and [*torch._dynamo.explain*]() are useful for debugging graph structures and breaks. Alternatively, one can wrap function calls in a try-except block to enforce strict behavior and catch graph break exceptions early, aiding in the identification of performance bottlenecks at the Python-level. *torch.compile* also has a parameter [*fullgraph (boolean)*](), controlling the tracing behaviour. If the argument is set to *False (default)*, it identifies and compiles capturable regions in the function. If *True*, the entire function must be compiled as a single graph. 
...
Starting in the middle of the diagram, AOTAutograd dynamically captures autograd logic in an ahead-of-time fashion, producing a graph of forward and backwards operators in FX graph format.... MLIR is integrated into specific components of PyTorch, particularly in TorchDynamo and TorchInductor, to improve performance and enable hardware-specific code generation....

Hence the FX graph (i.e. the container of a program) becomes key element of Torch IR. Using Aten and Primitive Operations (PrimTorch), Torch IR is converted to a lower-level representation, targeting the backend for specific hardware. In some cases, the computations in the Torch IR are replaced with more efficient primitives. PrimTorch are a set of highly optimized and hardware-specific operations that can further speed up the computation, particularly on GPUs. They are often used to replace more general operations with more specialized ones that take better advantage of hardware features.
