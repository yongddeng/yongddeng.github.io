...
A computer memory is essentially an array of bytes each with a unique address. That is, [random access memory]() (RAM), register, and cache are all some abstractions. The latency of  [dynamic RAM]() (DRAM) can be estimated as $T_{\text{precharge}} + T_{\text{row activation}} + T_{\text{col. access}}$. The access is fastest when data is in an active row (i.e. as contiguous) while the physical design of memory cells really matters. [Static RAM]() (SRAM) is better suited for cache memory due to its high speed and stability, despite its higher cost. A disk can be sometimes used as [virtual memory](), via [paging]() or [segmentation](), to free up RAM for a process, but excessive swapping can lead to [disk thrashing]().
