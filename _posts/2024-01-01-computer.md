...
The emergence of Triton and the dominance of PyTorch over TensorFlow might share a common root cause (i.e. an naive subjective interpretation). Compilers, despite decades of refinement—for example, GCC for C—remain challenging to use, particularly for ML researchers and practitioners who often prefer tools that align with the behavior of interpreters. This preference explains why [CPython]() (i.e., Python) gained widespread adoption: its dynamic nature, intuitive syntax, and extensive ecosystem of open-source libraries cater to the iterative and experimental workflows.
...
Modern interpreters typically integrate virtual machines (VMs) to facilitate JIT compilation. For instance, CPython compiles "hot regions" (i.e. frequently executed code paths) of source code (i.e. .py) into a platform-independent [bytecode]() (i.e. .pyc in the \_\_pycache\_\_ dir.) to balance performance and responsiveness. This bytecode is then sequentially executed by the stack-based [Python VM]() (PVM), which operates at runtime by interacting with the operating system and underlying hardware. The *dis* module allow developers to inspect bytecode, revealing the IR generated from source code (e.g. a string or script). 
