...
The [Triton compiler]() captures the AST of a kernel written in *triton.language* and decorated by *@triton.jit*, then progressively generates IRs through appropriate MLIR dialects: i) Triton Tensor Graph IR (TTGIR): a hardware-agnostic IR for high-level opt.; ii) Triton-GPU IR: a hardware-specific IR optimised for efficient GPU code generation; and iii) LLVM IR: LLVM lowers validated IRs into an executable; Triton offers finer control over SMEM and HBM compared to eager mode PyTorch (i.e. prior to version 2.0). The throughputs of the fused softmax in Trition seems higher than both the *F.softmax* and PyTorch-native implementation invoked with  *@torch.jit.script*.
