...
An instance of a (user-) program, known as a [process](), resides in RAM during execution. The [text segment]() of the process, as depicted in its [memory layout](), contains the executable code including instructions (e.g. 4-bit or 32-bit instructions) which the CPU fetches and executes. To maintain program integrity and safeguard against accidental or malicious modifications, it is typically designated as read-only. Additionally, some modern systems employ hardware mechanisms, such as memory protection keys, to further restrict access. In most systems, the text segment is usually situated at the start of the process's virtual address space.
<!-- In modern systems, the text segment is often situated at the start of the process’s virtual address space. -->
