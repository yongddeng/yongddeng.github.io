...
Computing begins with machine language, the lowest level, consisting of [binary]() instructions interpretable and executable by the CPU. [Assembly]() languages, such as x86, ARM, and MIPS, introduced human-readable mnemonics which are translatable into machine code. Low-level languages (i.e. less closer to binaries than assembly) like C offer higher abstraction while maintaining hardware control. The evolution continued with high-level languages like Python, Java, and C++, which abstract hardware complexities and rely on [compilers]() or [interpreters]() (i.e. that may or may not use JIT compilation as its feature) to convert abstracted syntaxes into machine executable. 

This shift increased productivity, enabling complex systems without requiring deep knowledge of binary encoding or CPU instructions. I/O devices, especially  {I: 'keyboard', O: 'monitor'}, further facilitate communication between a computer and its external environment. Each key press is translated into a corresponding binary based on encoding standards. The [unicode transformation format-8]() (UTF-8) is variable-length which represents characters as sequences of 8-bit bytes, supporting over a million codepoints. It is a superset of all characters in widespread use today, including those defined by Hex and ASCII (i.e. 7-bit encoding that only supports 128 characters).
...
Similar to CPUs, a [streaming multiprocessor]() (SM) serves as a core of the GPU which executs a thread block. Given that a [thread]() is the smallest unit of execution that handles a portion of tensor operation (e.g. element-wise mul.), an [warp]() is a set of threads organised in a uniform size to be executed under the SIMT model, and a [block]() is a collection of warps assigned to each SM. If more blocks than the number of SMs can be properly dispatched in modern GPUs, then [warp schedulers]() will facilitate the switching between warps within the SMs whenever a core stalls, and enables maximum utilisation via effective latency masking and higher throughput.
...
Writing compilers are unpleasant to perhaps the most programmers.... It requires .... The currently existing compilers such as GCC (for C) have been improved over 20 years of research and feedback.... 

<!-- - but machine code can do a way more optimisations that are not possible with just bytecode (especially for loops)....  -->

Interpreters in the previous days used a tree (similar to AST) to represent source code and [recursion]() for execution. However, this tree was primarily designed for representation (i.e. not execution), thus lowering to machine code is difficult as it is too complicated. More fashionable interpreters employeed a [virtual machine]() (i.e. VM) for execution. In particular, the default Python, [CPython](), use a stack-based Python VM (i.e. PVM). bytecode being transformed from the AST is PVM executable. The *dis* module can be used to check the bytecode originated from source code (i.e. string). 
The bytecode interpreters are better than the old ones, but still ....

JIT compilation.... Optimisations possible due to JIT are \[register allocation, instruction scheduling, constant folding, dead code elmination, common subpression elmination\].... JIT compilation vs naive execution... jit compilation is slow, almost as slow as native (AOT) compilation... so we only compile hot region..... Most ML practitioners still prefer ... Having understood the compilation workflows would benefit ... PyTorch is preferred by researchers as its UX can keep the nature of doing Python programming... PyTorch (and other frameworks, not only limited to ML) attempted to ... 

<!-- JIT: compile native code during runtime for hot regions; balance startup time vs execution time;  -->
