...
The LibTorch was further used to enable [script mode]() via two main APIs. [*torch.jit.trace*]() builds a SCG using tensor operations executed on fixed inputs during a forward pass, valid only for models with expected paths (i.e. fixed control flows). Beside the tracer, Fb added [*@torch.jit.script*]() for an input-dependent (i.e. dynamic) model compilation directly from Python code into [TorchScript]() IR, such that any TorchScript programs can be saved from Python and loaded into non-Python env. (e.g. train.py $\to$ deploy.cpp). Dynamic models may be exported to the [Open Neural Network Exchange]() (ONNX) format, though they must comply with ONNX's static graph requirements.
