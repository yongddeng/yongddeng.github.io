...
[AOTAutograd]() optimises gradient computation for the captured subgraphs, particularly for functions requiring backpropagation. Once Dynamo identifies a subgraph, AOTAutograd takes it and splits it into forward and backward (static-) computation graphs. By treating the forward and backward passes as distinct subgraphs, AOTAutograd can optimize the forward pass independently for further optimisations (e.g. operation fusion, redundant computation elimination). It can also generate a minimal backward pass (which is often more computationally expensive) tailored to the forward pass, as performance is tightly coupled to the efficiency of the forward pass. 

By treating the forward and backward passes as distinct subgraphs, AOTAutograd can: optimize the forward pass independently. It includes fusing operations, eliminating redundant computations, or reducing memory footprints. Moreover, it can generate a minimal backward pass tailored to the forward pass....
Ahead of time backward pass (i.e. a SCG construction) is in contrast to the PT 1.x's approach where the backward pass is dynamically generated during runtime. During the forward pass, intermediate values (e.g. activations) are usually saved for use in the backward pass. 
