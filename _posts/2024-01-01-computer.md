...
The [Python virtual machine]() (PVM), a stack-based interpreter, is responsible for executing bytecode at runtime by simulating a CPU's execution model (i.e. stack machine). In particular, the (legacy API-) *PyFrame_New* in Objects/frameobject.c returns a [*PyFrameObject*]() that models a [stack frame]() and holds the state of the executing bycode (e.g. references to the code, local and global namespaces, and stack-related metadata). A frame object has i) *[f_code]()*: a reference to the *PyCodeObject*; and ii) *[f_back]()*: a link to the last frame;, where the latter constructs the [linked list]() of *PyFrameObject* pointers. The Python built-in *inspect* module helps examining the two objects.
