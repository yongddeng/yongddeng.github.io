...
An [iteration-level scheduling]() (i.e. continuous batching), proposed by [Yu et al. (2022)](), reduces latency by determining the batch size at each iteration. However, existed systems reserved contiguous memory spaces for KV cache, leading to wasteful
 slots, internal-external fragmentation, and thus limiting throughput. [Kwon et al. (2023)](https://arxiv.org/abs/2309.06180) addressed the issue with i) [PagedAttention](), a non-contiguous physical memory page on VRAM, inspired by virtual memory and paging; ii) flexible KV cache sharing across and within requests; They open-sourced an LLM serving framework [vLLM](https://github.com/vllm-project/vllm), on top of the mentioned techniques along with other techniques mentioned in this section.
