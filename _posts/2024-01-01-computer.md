...
However, TS cannot fully support Python features (e.g. generator, comprehension), but force external libraries that are not compatible with TS to be wrapped or avoided. To remedy, the [*torch.compile*]() API was introduced as the heart of [Pytorch 2.0]() (PT2). This is domain-specific, unlike general-purpose compilers (e.g. GCC, LLVM), which identifies general Python code and PyTorch-related code within a program, and represents the latter as a DCG. Technically, *torch.compile* conducts the following procedures: i) graph acquisition (i.e. tracing); ii) graph lowering (i.e. high-level optimisation); and iii) seamless back-end compatibility along with prepackaged compilers.
