...
A [central processing unit]() (CPU or processor) executes instructions, known as [programs](https://youtu.be/RU1u-js7db8?si=cGIkhRbh6XW_mr34), via the [fetch-decode-execute]() cycle. Modern CPUs feature multiple cores (e.g. 4 to 64) along with a [floating-point unit]() (FPU) for efficient real-number computation, while each core includes its own [arithmetic logic unit]() (ALU) and [control unit]() (CU). CPUs also possess dedicated memory modules. <!-- CPUs have dedicated memory modules --> The fastest form of memory, called the [register](), temporarily holds data and instructions immediately required by the CPU, while its [cache hierarchy](), often comprising {'L1': smallest-fastest, 'L2': larger-slower, 'L3': largest-shared across cores}, mitigates latency from slower main memory.

Given [clock speed]() refers to how quickly instructions are issued per second, a CPU’s [word size](http://cpuville.com/Projects/Original-CPU/programming.html) refers the number of bits it can process in a single operation, and modern CPUs commonly employee 32-bit or 64-bit. Word size affects register width, data path size, and the design of the [instruction set architecture](https://www.slideshare.net/slideshow/instruction-set-architecture-254097280/254097280#2) (ISA), where instructions (represented in [binary]()) abstract the underlying hardware and supply a software interface. <!-- to the processor --> Using data types that align with the native word size can improve performance and reduce instruction count, but programming languages (e.g. C) define only minimum type sizes (e.g. int $\geq$ 16 bits) to maintain portability <!-- across architectures --> across architectures. 
