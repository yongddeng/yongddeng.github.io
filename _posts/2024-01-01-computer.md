...
PyTorch is an open-source deep learning (DL) framework developed by Facebook, evolving from the [Torch7]() library. PyTorch 1.x build [dynamic computation graph]() (DCG) for flexible prototyping and easy debugging, while TensorFlow 1.x make [static computation graph]() (SCG) for efficient serialisation and deployment. They use [*torch.Tensor*]() class as the core data structure representing homogeneous, multi-dim. rectangular arrays. It enables [automatic differentiation]() (AD) within a DCG using the key attributes: [*'requires_grad', 'grad', 'grad_fn'*]. Similar to *numpy.array*, a tensor also contains metadata which describe its shape: [*'size', 'stride', 'dtype', 'device', 'layout'*].
 
<!-- Torch was implemented in [Lua]() for scientific computing. -->

A [*torch.autograd.Function*]() is used to define non-native PyTorch operations which require custom forward and backward computations, although PyTorch provides various built-in Function classes (e.g. AddBackward0, MulBackward0) which handle [gradients]() for standard operations. It updates a DCG at each execution of an operation with tensors. That is, *torch.Tensor* and *torch.autograd.Function* can be considered respectively as nodes and edges of a graph which carries data and encapsulates the logic for bidirectional calculations. It facilitates the dynamic propagation within a neural network (NN) at run times, and one may use *Tensor.register_hook* to act with flows.
...
PyTorch 1.x heavily relied on its C++ backend to address the slower execution associated with [eager mode](https://pytorch.org/blog/optimizing-production-pytorch-performance-with-graph-transformations/). The Python API primarily served as a high-level interface, delegating tasks to lower-level modules designed for AD and tensor operations. In particular, [A tensor library]() (ATen) is a C++ wrapper for C and CUDA libraries such as the [math kernel library]() (MKL), [CUDA deep neural network]() (cuDNN), and [CUDA basic linear algebra subprogram]() (cuBLAS). While these lowest-level modules handle primitive tensor operations, they lack native AD supports, and hence ATen incorporated autograd engines, preserving flexibility while leveraging the performance of CUDA.

[Alban Desmaison](https://pytorch.org/docs/stable/community/persons_of_interest.html) commented: "Given that Torch7 (i.e. written in [Lua]()) relied on these C/C++ libraries: Tensor Handling (TH/THNN) and Tensor Handling CUDA (THC/THCUNN being CUDA dependent) for CPU/GPU computing, i) the initial PyTorch was simply a Python wrapper + new autograd engine + TH\*; ii) Aten was developed later to provide C++ interfaces, allowing new PyTorch functionalities to be implemented within ATen rather than TH\*; iii) [LibTorch]() is essentially the distribution of PyTorch, a replacement and/or supplmenet for the Python wrapper (which used [Pybind11]() to wrap ATen), enabling more direct access to the core backends through the C++ API".
...
LibTorch was also utilised to enable [script mode]() via two main APIs. [*torch.jit.trace*](), that aims to compile models with fixed control flows, builds a SCG using tensor operations executed on fixed inputs during a forward pass. Beside the tracer, [*@torch.jit.script*]() was featured for an input-dependent (i.e. dynamic) model compiliation. It lexes and parses entire Python code and convert them into a TorchScript IR, where any TorchScript programs can be loaded into non-Python environments (e.g. train.py $\to$ deploy.cpp). A dynamic model may be exported to the [Open Neural Network Exchange]() (ONNX) format, though they must comply with ONNX's static graph requirements.
...
The [*Torch.compile*]()  API has been advertised for [Pytorch 2.0]() (PT2). Unlike general purpose compilers like GCC and LLVM, *torch.compile* is domain-specific. It distinguishes python code and PyTorch-related code within a program, and transform the latter into computation graphs as other DL compilers do. As graphs are the container of compilable operations which downstream compilers will optimise and transform into an executable, Facebook hevaily invested resources to develop underlying tools for i) graph acquisition (i.e. tracing); ii) graph lowering (i.e. high-level optimisation); and iii) back-end compatibility along with prepackaged machine executable generators.
...
A [PyFrameObject]() represents a single execution frame in Python which tracks the execution state of Python code (e.g. local and global variables, the call stack, the instruction pointer). Whereas, a [PyCode Object]() represents a block of compiled Python code containing constant, variable name, and others needed for execution (but does not contain execution state). The latter is immutable and reused across multiple calls to the same function/block. A PyFrameObject has a reference to its corresponding PyCodeObject via the *f_code* attribute. The latter defines what code to execute, while the former manages how it is being executed. TF 2.0 keeps this behaviours. 
