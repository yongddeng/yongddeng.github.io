...
![overview](https://www.youtube.com/watch?v=d86ws7mQYIg)
...
A computer memory is basically an array of bytes each with a unique address. That is, an [random access memory]() (RAM), register, and cache can be viewed as unique abstractions. [Dynamic RAM]() (DRAM) latency depends on data access patterns and physical design of memory cells. Access is fastest when data is within an active row (i.e. contiguous) while latency could increase by $T_{\text{precharge}} + T_{\text{row activation}} + T_{\text{column access}}$ when accessing a new row. [Static RAM]() (SRAM), known for its high speed and stability despite its higher cost, is therefore appropriate for cache. Notice that a portion of persistent storage could be allocated as [virtual memory]() when a process exceeds the memory limits of RAM.
...
![HDD vs SSD](https://www.backblaze.com/blog/wp-content/uploads/2018/03/hdd_vs_ssd_bz.png)
