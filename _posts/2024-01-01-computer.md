...
Facebook utilised LibTorch and added *torch.jit.trace* to support JIT compilation. It records operations during a forward pass with fixed inputs, producing a SCG that is ideal for models with predictable execution paths (i.e. control flows may break compilation). They also added *torch.jit.script* which directly compiles Python code, allowing input-dependent models. Any [TorchScript]() program can be saved from a Python process and loaded in a process where there is no Python dependency (i.e. train.py $\to$ deploy.cpp). Models may be exported to the [Open Neural Network Exchange]() (ONNX), a standardised format for cross-platform deployment, while dynamic models may need adjustments to conform to ONNX's static graph requirements.
