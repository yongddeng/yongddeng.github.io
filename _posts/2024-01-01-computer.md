...
Subsequently, JIT compilers are introduced to further accelerate execution speed. Given that AOS (i.e. general-purpose) compilation could be slow, as [Omer Iqbal](https://www.youtube.com/watch?v=sQTOIkOMDIw) mentioned in his solid presentation, traditional JIT compilers, found in other languages such as Java (JVM) or JavaScript (V8 engine), employ multi-tiered optimisation strategies, such as the 'warm' or 'hot' stages, which identify more frequently executed code paths. The code regions are compiled into machine code during runtime. While CPython, the reference implementation of Python, does not have such a feature, one can find other Python implementations incorporating JIT compilation.
