...
Facebook utilised LibTorch and added *torch.jit.trace* to support JIT compilation. It records operations during a forward pass with fixed inputs, and emits a SCG that is ideal for models with predictable execution paths (i.e. control flows). They later added *@torch.jit.script*, which directly compiles Python code into [TorchScript]() IR, for input-dependent models. While any TorchScript programs can be saved in Python and loaded in a non-Python environment (e.g. train.py → deploy.cpp), a PyTorch model could be exported to the [Open Neural Network Exchange]() (ONNX), though dynamic models may require adjustments to comply with ONNX's static graph requirements.
