...
[TorchDynamo]() (i.e. Dynamo) is the front-end compiler designed for Python-level graph acquisition. It leverages PEP 523 (i.e. the frame evaluation API) and [functional transformation]() (FX) toolkit. Dynamo initially i) hooks into bytecode execution; ii) analyses bytecode with proxies and identifies torch.\* and non-torch.\* calls; and iii) transform compilable regions into an [FX graph]() (i.e. *torch.fx.GraphModule*); Assume a mock .py of {[torch.* ops, torch.tensor]: TorchVariable, Py\_builtin\_variables: BuiltInVariable, Py\_lists: ListVariable}, then the operations on a [TensorVariable]() adds a FX node in the graph, and can be drawn by *torch.fx.symbolic_trace* (or, *torch._dynamo.export*).
...
[Graph breaks]() can occur when Dynamo encounters complex behaviours. For example, [dynamic control flow]() refers to situations where the path of execution (which operation to apply) depends on runtime data. If one has *condition = lambda x: x.mean() > 0.5* that relies on the values held in *x: torch.Tensor*, then *if condition(x): return x / 1.1; return x / y;* will be broken down into two seperate FX graphs, leading Dynamo to trace the computation for each case. Frequent graph breaks often mean [performance hit]() due to guards. APIs like *torch._dynamo.explain* and *torch.fx.graph_module.GraphModule.print_tabular* can be helpful for diagnosing and debugging breaks. <!-- Dynamo will revert back to  -->
...
FX graphs (i.e. of torch.\*) produced by Dynamo accomodate guards and so the downstream compilers find them difficult to work with. Also, the nodes in FX graphs (i.e. tensor operations) must have *grad_fn* to be *.backward* callable, while the backward pass is generally more expensive and tightly coupled to the efficiency of the graph representing the forward pass. [AOTAutograd]() (i.e. AOT), which lives under the *torch._functorch.aot_autograd* module, decomposes all IR nodes into functional ATen operators being closer to hardware. It relies on [\_\_torch_dispatch\_\_]() hooks to intercept tensor-level operations, and works in conjunction with Dynamo for better traceability.

AOT traces the forward (i.e. FWD) and backward (i.e. BWD) passes as a joint graph, and then partitions the graph into two separate FX graphs (i.e. of aten.\*).... [PrimTorch]() (i.e. torch/_prims) is a minimal abstraction of ATen specific for graph-based workflows. It reduced ~2k+ PyTorch operators down to a closed set of ~250 primitive operators.... AOT can employ techniques like activation checkpointing or recompute-on-demand to manage this memory-pressure tradeoff....
...
- <div style="position: relative; display: inline-block;"> <img src="../assets/blog/2024-01-01-torchinductor.png" width="500"> </div> <a href="https://hc2023.hotchips.org/assets/program/tutorials/ml/PyTorch%202.0.pdf" target="_blank" style="position: absolute; bottom: 8px; right: 4px; font-size: 12px;">[src]</a> </div>
- <div style="position: relative; display: inline-block;"> <img src="https://pytorch.org/assets/images/training-production-ai-models/blog-fig1.jpg" width="500" height="138"> <a href="https://pytorch.org/blog/training-production-ai-models/?utm_source=social-facebook&utm_medium=m4d&utm_campaign=organic&utm_content=post-url&utm_offering=artificial-intelligence&utm_product=training-production-ai-models_01302024&eventSource=organicsocialm4d&content_id=FmvhyYO3MCBLU8r" target="_blank" style="position: absolute; bottom: -8px; right: 4px; font-size: 12px;">[src]</a> </div>
