...
OG interpreters, which traversed ASTs recursively during runtime, suffered from performance limitations. ASTs were well-suited for representing program structure, but they were not optimised for efficient execution. The introduction of a VM and bytecode as input significantly improved performance and portability, while more advanced optimisation techniques or custom implementations can be done to a lower-level IR. For instance, [PEP 3147]() introduced the .pyc repository directory, called [\_\_pycache\_\_](https://peps.python.org/pep-3147/), to better organise files and improve cross-version compatibility. CPython skips recompilation if an up-to-date .pyc file is present in the directory.
...
Likewise, the introduction of JIT compilers aims to further accelerate execution. Given that AOS (i.e. general-purpose) compilation can be slow, as [Omer Iqbal](https://www.youtube.com/watch?v=sQTOIkOMDIw) mentioned in his solid presentation, traditional JIT compilers, found in other programming languages such as Java (JVM) or JavaScript (V8 engine), employ multi-tiered optimisation strategies, such as the 'warm' or 'hot' stages, which identify frequently executed code paths. These code regions are compiled into machine code during runtime. While CPython, the reference implementation of Python, does not have such a feature, one can find other Python implementations incorporating JIT compilation.
