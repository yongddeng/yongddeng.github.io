...
Note that Dynamo is a Python-level JIT compiler which dynamically transforms operations into FX graphs and accelerates forward (i.e. FWD) passes. Whereas, [AOTAutograd]() (i.e. AOT) is specifically designed to optimise backward (i.e. BWD) passes by decomposing the AD computations into smaller, lower-level operations. These decompositions are then compiled using a backend compiler (e.g. Triton). Internally, AOT uses *\_\_torch_dispatch\_\_*-based tracing to extract both FWD and BWD graphs, and wraps them in *torch.fx.GraphModule* containers.... This tracing mechanism is clearly different from the symbolic tracing of Dynamo....
...
The components of PT2 compilation stack, such as Dynamo and AOT, can be used independent, but they are complementary rather than exclusive. If AOT is used without Dynamo, it does not capture Python control flow (e.g. loop, condition, or dynamic branching). Instead, it focuses only on the tensor operations executed during the eager-mode forward pass, making it less powerful in acquiring full program graphs compared to Dynamo.... On the other hand, if Dynamo is used standalone, it does not further optimise its FX graph in Torch IR, nor construct one for backward computations. By default, *torch.compile* uses Dynamo AOT are used together....
