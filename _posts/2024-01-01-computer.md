...
[Kwon et al. (2023)](https://arxiv.org/abs/2309.06180) open-sourced [vLLM](https://arxiv.org/abs/2309.06180), introduces two key ideas: i) vLLM employs continuous batching, dynamically updating request batches during inference, allowing for more efficient GPU resource usage and faster early results; ii) the PagedAttention algorithm, inspired by virtual memory and paging techniques, enables efficient non-contiguous storage of attention keys and values, optimizsing memory usage and reducing fragmentation; vLLM also supports quantization, along with optimised CUDA kernels for GPU performance, streamlining KV cache management and memory access. 
