...
As the maximum performance gain comes from the minimal graph breaks, one can force TorchDynamo to raise an error upon the first graph break using the try-except block. Apparently, *torch.compile* has the *fullgraph (bool)* parameter. With its default backend (i.e. Inductor), if the argument is *False (default)*,  *torch._dynamo* searches compileable regions in the function. If set to *True*, the entire function must be capturable as a single graph (i.e. no graph breaks), otherwise it will raise an error (i.e. other backends may stay quite for such issues). *torch.fx.graph_module.GraphModule.print_tabular*, or *torch._dynamo.explain* may help debugging.  <!-- this Python-level JIT compiler -->
...
- E.g. ...
        ...
        if __name__ == '__main__':
              ...
              # dynamo.reset()  # reset to use a custom backend (i.e. not Inductor).
              # my_fn_opt = dynamo.optimize(custom_backend)(data_dependent_fn)(x, y)
        
              explain_output = dynamo.explain(my_fn, x, y)
              attributes = [
                   attr for attr in dir(explain_output) if not "__" in attr
              ]
              ...
              for attr in attributes:
                   ...
                   if isinstance(value, list) and len(value) >       0 and hasattr(value[0], "graph"):
                          print(f"{attr}: (contains graphs)", *      [f"\n  Graph {i}:\n{g.graph.      print_tabular()}" for i, g in       enumerate(value)])
                  else:
                          print(f"{attr}: {value}")
- res ... 
