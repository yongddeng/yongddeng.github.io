...
[Alban Desmaison](https://pytorch.org/docs/stable/community/persons_of_interest.html) noted: "Given that Torch7, originally written in [Lua](), used the C/C++ libraries: Tensor Handling (TH/THNN) and Tensor Handling CUDA (THC/THCUNN) for CPU/GPU computations, where THC* had CUDA/cuDNN dependencies, i) the Initial PyTorch was a Python wrapper + new autograd engine + TH*; ii) Aten was developed to provide C++ (cpp) interfaces, thus adding functionalities (i.e. tensor operations) to PyTorch can be done in ATen; iii) [LibTorch](), standalone C++ API, was developed as a replacement for the Python wrapper (which used [Pybind11]() for wrapping), and LibTorch users can directly exposes the C++ API without involving Python".

Facebook utilised LibTorch and added *torch.jit.trace* to support JIT compilation. It records operations during a forward pass with fixed inputs, producing a SCG that is ideal for models with predictable execution paths (i.e. control flows breaks compilation) Furthermore, they introduced *torch.jit.script* (i.e. TorchScript) which directly compiles Python code, supporting input-dependent behaviours of a model. However, if both APIs are insufficient, models can be exported to the [Open Neural Network Exchange]() (ONNX), a standardised format for cross-platform deployment, while dynamic models may need adjustments to conform to ONNX's static graph requirements.
