...
The main entry point for code compilation in CPython is the *_PyAST_Compile* function in [Python/compile.c](https://github.com/python/cpython/blob/main/Python/compile.c#L1371). It returns a *[PyCodeObject]()* struct that encapsulates the compiled bytecode for a function or block of code. This object takes parameters such as i) *co_code*: the sequence of opcodes and operands representing instructions; ii) *co_consts*: constants (e.g. number, tuple) or nested *PyCodeObject* instances (e.g. function, class, comprehension); iii) *co_names*: variable and attribute names; iv) *co_varnames*: local variable names; v) *co_freevars* / *co_cellvars*: variables used in closures; and vi) *co_filename* / *co_name*: the source file and code block name; 

The [Python virtual machine]() (PVM), a stack-based interpreter, is responsible for executing bytecode at runtime by simulating a CPU's execution model (i.e. stack machine). In particular, the (legacy API-) *PyFrame_New* in Objects/frameobject.c returns a [*PyFrameObject*]() that models a [stack frame]() by holding the state of the executing code (e.g. references to the code, local and global namespaces, and stack-related information). A frame object has i) *[f_code]()*: a reference to the *PyCodeObject*; and ii) *[f_back]()*: a link to the last frame;, both of which are used to form a [linked list]() of *PyFrameObject* pointers. The Python built-in *inspect* module helps examining the two objects.

Finally, the *_PyEval_EvalFrameDefault* function in Python/ceval.c serves a main interpreter loop, which, as per [PEP 523](), is configurable. Note that "all" Python objects and data structures are stored in a private heap, while *PyFrameObject* holds the execution context, and a *PyCodeObject* act as a blueprint. Its [call stack]() is a basic stack of references (i.e. conceptually stack-allocated). \*\**In recent updates, Python 3.11 splits the stack frame object into: a PyFrameObject and a struct [_PyInterpreterFrame](). Moreover, Python 3.12, as proposed in [PEP 709](https://peps.python.org/pep-0709/), inlines comprehensions into the code instead of compiling them as nested functions that isolate iteration variables*.\*\*
