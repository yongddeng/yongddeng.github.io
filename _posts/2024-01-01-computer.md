...
Apparently, minimising graph breaks is important for the high performance. [*torch.fx.graph_module.GraphModule.print_tabular*]() and/or [*torch._dynamo.explain*]() help debugging graph structures and breaks. Or, one may wrap function calls in a try-except block to enforce strict behaviour and catch graph break exceptions early, aiding in the identification of performance bottlenecks in the Python-level. *torch.compile* also has a parameter [*fullgraph (boolean)*]() to control the tracing behaviour. If the argument is *False (default)*, it identifies and compiles capturable regions in the function. If set to *True*, the entire function must be compiled as a single, uninterrupted graph. 
