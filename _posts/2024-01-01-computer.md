...
I/O tools increased productivity, enabling complex systems without requiring deep knowledge of binary encoding or CPU instructions. In particular, {I: 'keyboard', O: 'monitor'}, further facilitate communication between a computer and its external environment. Each key press is converted into a corresponding binary using encoding standards. The [unicode transformation format-8]() (UTF-8) is a variable-length standard which representes over a million codepoints as sequences of 8-bit bytes. It is a superset of all characters in widespread use today, including those defined by [Hex]() and [ASCII]() (i.e. 7-bit encoding standard merely supporting 128 English letters).
...
Computing begins with machine language, the lowest level, consisting of [binary]() instructions interpretable and executable by the CPU. [Assembly]() languages, such as x86, ARM, and MIPS, layered human-readable mnemonics that can be translated into machine code by assembler. Low-level languages (i.e. farther from hardware) like C offer higher abstraction while maintaining hardware control, and use [compilers]() to translate [source code]() into an executable. The evolution continued with high-level languages including Python, Java, and C++. They even abstract hardware complexities and use compilers or [interpreters]() (i.e. may or may not use a JIT compile stack).

The choice of programming langugaes depend on ....
...
The [multi-level intermediate representation]() (MLIR), located in llvm-project/mlir, is a framework placed between a language's AST and LLVM IR, enabling different levels of abstraction to coexist. While it is a powerful representation, MLIR does not aim to provide low-level machine code (e.g. .exc file) generation algorithms (e.g. register allocation, instruction scheduling), as such tasks are handled by lower-level optimisers (i.e. LLVM). Nor does it intend to serve as a source language for user-defined kernels, analogous to CUDA C++. MLIR uses [dialects]() to represent the semantics of both general-purpose and [domain-specific languages]() (DSLs) as first-class entities.
...
[Compute unified device architecture]() (CUDA) is a parallel computing platform featuring compilers, libraries, and developer tools. It was created in 2006 by NVIDIA and embedded within C/C++ as a DSL for programming their own GPUs. [NVIDIA CUDA Compiler]() (NVCC) which uses NVVM (i.e. NVIDIA's derivatvie of LLVM), compiles a .cu into a i) [parallel thread execution]() (.ptx) human-readable assembly; or ii) directly executable [CUDA-specific binary]() (.cubin); The .cu extension allows NVCC to delegate the compilation of [host code]() (i.e. CPU-related: *#include*) to Clang, and focus on compiling [device code]() (i.e. code with CUDA extension: *\_\_global\_\_*, and *\_\_device\_\_*).
...
Cross-compiling has become more accessible with LLVM, but despite extensive research and expertise, creating compilers remains challenging and limits their widespread use. Accordingly, interpreters have gained popularity among users who favour iterative as well as experimental workflows. Modern interpreters convert source code into [bytecode](), which is i) hardware-agnostic: independent of the CPU's [instruction set architecture]() (ISA); and thus ii) executable on any platform (e.g. Linux, macOS, Window) with a compatible [virtual machine]() (VM). Compilation is often faster than -to binary, and they also execute instruction sequentially, making them highly dynamic.

The [CPython]() compiler's design is outlined in [PEP 339](https://peps.python.org/pep-0339/), but in essence, its role is to transform source code (i.e. .py) into bytecode (i.e. .pyc), which consists of [operation codes]() (opcodes) and their corresponding [operands]() (i.e. if exists). Given that opcodes are low-level instructions, operands represent various entities depending on the opcode's context, such as literal constants or immutables (e.g. number, string, tuple), variable or attribute names, or code blocks (e.g. function, classe, comprehension). Each Python mnemonic corresponds to an integer of opcode, highly similar to how assembly instructions like *pop*, *push*, and *ret* are represented numerically.

Bytecode, formed by combining opcodes and operands, serves as an IR that the CPython interpreter can efficiently execute. However, CPython continues to improve its performance through various optimisations. For instance, the [adaptive interpreter]() introduced in [PEP 659](https://docs.python.org/3/whatsnew/3.11.html#whatsnew311-pep659) dynamically replaces generic opcodes with specialised, faster versions at runtime based on profiling data. This reduces overhead for frequently executed instructions, and so significantly enhances execution speed. One can indeed disassemble Python opcodes using the *dis* module or examine further from the CPython source code: [[Include/opcode.h](https://github.com/python/cpython/blob/main/Include/opcode.h), [Python/opcode_targets.h](https://github.com/python/cpython/blob/main/Python/opcode_targets.h), [Lib/opcode.py](https://github.com/python/cpython/blob/main/Lib/opcode.py)]. 
...
The main entry point for code compilation in CPython is the *_PyAST_Compile* function in [Python/compile.c](https://github.com/python/cpython/blob/main/Python/compile.c#L1371). It returns a *[PyCodeObject]()* struct that encapsulates the compiled bytecode and its associated metadata. For the interpreter, this custom object takes parameters, including i) *co_code*: the sequence of opcodes and operands representing bytecode instructions; ii) *co_consts*: constants (e.g. string, number, tuple) or nested *PyCodeObject* instances; iii) *co_names*: variable and attribute names; iv) *co_varnames*: local variable names; v) *co_freevars* / *co_cellvars*: variables used in closures; and vi) *co_filename* / *co_name*: the source file and code block name; 

The CPython uses the [Python virtual machine]() (PVM), a stack-based interpreter, to execute bytecode at runtime by simulating a CPU's execution model (i.e. stack machine). In particular, the (legacy API-) *PyFrame_New* in Objects/frameobject.c builds a [*PyFrameObject*]() and models a stack frame for the current code being executed. This object mainly contians i) *[f_code]()*: a reference to the *PyCodeObject* being executed; ii) *[f_back]()*: a link to the previous frame; These frames form a [linked list]() that the PVM manages via recursion, while the [*PyEval_EvalFrameEx*]() function (i.e. [*PyEval_EvalFrameDefault*]() in Python 3.9+), located in Python/ceval.c, is the main interpreter loop.

<!-- (i.e. In Python 3.11, the frame objects are splited into two pieces: the full frame object used in all previous versions and a new internal C struct that relies entirely on external system).  -->

<!-- Furthermore, comprehensions are currently compiled as nested functions, which provides isolation of the comprehension’s iteration variable, but is inefficient at runtime. PEP 709 proposes to inline list, dictionary, and set comprehensions into the code where they are defined, and provide the expected isolation by pushing/popping clashing locals on the stack -->

In summary, when *PyEval_EvalFrameEx* encounters a *PyFrameObject*, it processes opcodes one at a time, interpreting bytecode within the context of the frame. In particular, a *PyCodeOjbect* serves as a blueprint, while Python stack frames allocated in [heap memory]() can outlive its function call, and the *inspect* module can be used to examine frame and code objects (*Recently, Python 3.11 splits the frame object into: the legacy *PyFrameObject* and the internal C struct [*_PyInterpreterFrame*](). Moreover, Python 3.12, as proposed in [PEP 709](https://peps.python.org/pep-0709/), inlines list, dict, and set comprehensions into the code rather than compiling them as nested functions that isolate iteration variables*).

<!-- (i.e. the recent [PEP 709](https://peps.python.org/pep-0709/): the nested-function compilation of comprehensions optimises for compiler simplicity at the expense of performance of user code; Comprehensions are currently compiled as nested functions, which provides isolation of the comprehension’s iteration variable, but is inefficient at runtime. This PEP proposes to inline list, dictionary, and set comprehensions into the code where they are defined, and provide the expected isolation by pushing/popping clashing locals on the stack) -->

<!-- We know that all the static information about the program store in PyCodeObject. But what about the dynamic information when the program is running in the Python VM? In Python, there is a class to describe the envrionment at running time – PyFrameObject. It’s a simulation of stack frame in x86 platform. You noticed that PyFrameObject is a size-variable Python Object class. Because this class maintain a PyCodeObject and stack in different block have different size. So PyFrameObject can’t be a size fixed class. A set of PyFrameObject are organised like a single linked-list.  -->

<!-- The PVM’s main loop fetches bytecode instructions one at a time. It decodes each instruction and executes it using the appropriate internal function. Subsequently, each function call creates a new frame, which is pushed onto the frame stack. The PVM switches between frames to manage nested and recursive function calls (i.e. more will be discussed). Finally, The PVM resolves variable types, names, and attributes at runtime, allowing Python's dynamic features to function seamlessly. Actually, all the things that VM do is simulating what the OS do to excute a program. -->
...
[PEP 3147](https://peps.python.org/pep-3147/) introduced .pyc repository directories, called [\_\_pycache\_\_](), to better align clutter and enhance cross-version compatibility. If a .pyc file is available and up-to-date, Python skips the compilation step.... Historically, interpreters represented source code as ASTs and executed sequentially using an [recursion](). These [trees](), while well-suited for program structure representation, were not optimised for execution. Consequently, types of optimisation were limited, and lowering source code to machine code was also challenging....
<!-- compilation to Bytecode and uses of VM improved a little bit, but interpreters are still slow. -->
...
JIT compilers dynamically translate "hot region" (i.e. frequently executed code paths) of source code to machine code during runtime, enabling efficient executions.... JIT a part of the PVM in PyPy.... As mentioned during [Omer Iqbal](https://www.youtube.com/watch?v=sQTOIkOMDIw)'s great presentation, an AOS (i.e. general) compilation is heavy.... Nevertheless, other python implementations that equippted JIT is not so popular, and CPython is clearly slower than the other languages....

The PVM(PVM) in CPython, the reference implementation of Python, does not employ multi-tiered optimization strategies like "warm" or "hot" stages, which are typical of JIT compilers found in languages like Java (JVM) or JavaScript (V8 engine). Instead, the PVM executes Python bytecode using a straightforward interpreted approach, with limited optimizations.
...
TS does not support some libraries or Python features (e.g. generator, comprehension), but rather forces incompatible modules to be either wrapped/avoided. TS-compiled models also loss some of eager charm (i.e. PT's eager execution flexibility). Thus, [*torch.compile*]() API is introduced as a domain-specific compiler of [Pytorch 2.0]() (PT2). It transforms PyTorch-related regions into a SCG (as TS does), and continues to interpret general Python codel, maintaining its "pythonic" nature. They reported this dynamic trace (unlike TS-"once and for all") works 93% on the 163 open-source models, and the (partially-) compiled model runs 43% faster in training on the A100.
...
As known, the CPython compiler transforms source code into bytecode. A [CodeObject]() is generated during the compilation process to store the bytecode instructions along with static data (e.g. consts list, variable names, exception table) required for execution. If the *PyEval_EvalCode* function in [cpython/Python/ceval.c]() executes a CodeObject, a [PyFrameObject]() is created to hold the runtime state for that execution, where its *f_code* attribute points to the corresponding CodeObject (i.e. the CodeObject acts as a blueprint). By default, the *PyEval_EvalFrame* calls *_PyEval_EvalFrameDefault* at frame execution, but as per [PEP 523](), the evaluation function is adjustable.
...
Dynamo inserts a set of [guards]() (i.e. checkpoint markers) into the code to efficiently and collectively validate if the runtime conditions remain consistent with the assumptions made at JIT compilation (i.e. FX graphs construction). For instance, the shape guard triggers a recompilation when a tensor shape is shifted by a new batch size. It discards the cached graph and retraces the function to take in the new state.... Dynamo has 15 types of guards including [types, lists, attributes, dicts, consts, nn.Modules, mutations, etc]....  Dynamo guards also gurantee soundness of AOT Autograd tracing (i.e. backward tracing).
...
FX graphs (i.e. of Torch IR) produced by Dynamo accomodate guards and so the downstream compilers find them difficult to work with. Also, the nodes in FX graphs (i.e. tensor ops.) must have *grad_fn* to be *.backward* callable, while the backward pass is typically more expensive and tightly coupled to the efficiency of the graph representing the forward pass. [AOTAutograd]() (i.e. AOT), which lives under the *torch._functorch.aot_autograd* module, decomposes all IR nodes into functional ATen operators being closer to hardware. It relies on [\_\_torch_dispatch\_\_]() hooks to intercept tensor-level operations, and works in conjunction with Dynamo for better traceability.

Explicitly, \_\_torch_dispatch\_\_ is .... This is a diagram of how torch_dispatch might work with vmap, where the dotted arrows represent paths that could have been take. \_\_torch_dispatch\_\_ is the only way to go from C++ back into Python....  <!-- Note that AOTAutograd compiles computations (prior to execution) within the dynamic context of TorchDynam, so the term ahead-of-time is relative to the specific computation being intercepted and optimized at runtime -->
