...
In summary, CPython relies on a private heap containing "every" Python objects and data structures.  *PyCodeObject* act as a blueprint, and *PyFrameObject* which represent the [execution context]() can also outlive its [function call](). Unlike in other languages, the [call stack]() is the logical stack (that conceptually exists in stack memory) of *PyFrameObject* pointers. \*\**In recent optimisation, Python 3.11 splits the stack frame object into: the legacy PyFrameObject and the C struct [_PyInterpreterFrame](). Also, Python 3.12, as proposed in [PEP 709](https://peps.python.org/pep-0709/), inlines list, dict, and set comprehensions into the code instead of compiling them as nested functions that isolate iteration variables*.\*\*
