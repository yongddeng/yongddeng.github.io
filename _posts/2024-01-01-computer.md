...
[TorchDynamo]() (i.e. *torch._dynamo*) is the tracer, used in *torch.compile*, leveraging the [FX]() (i.e. Python-to-Python transformation toolkit) to produce clean graphs that are compatible with downstream compilers. For graph acquisition, Dynamo [hooks]() (i.e. intercepts) Python bytecode execution using the [Frame Evaluation API]() of CPython, and calls *torch.fx.symbolic_trace* to simulate execution on [proxies]() and identify capturable regions within a program, which recorded as nodes of an FX graph. Dynamo breaks the computation graph when it encounters unsupported Python features (e.g. data-dependent control flow), that CPython (i.e. default interpreter) can execute them.


<!-- The python code gen records captured operations as nodes in the FX graph.  -->

At a high-level, *torch.compile* receives *fullgraph (bool)* – If False (default), if attempts to discover compileable regions in the function that it will optimise. If True, then we require that the entire function be capturable into a single graph. If this is not possible (i.e. there are graph breaks), then this will raise an error... Other compilers might either raise errors or fail silently when encountering such issues. Tools like *torch._dynamo.explain()* and *TORCH_LOGS=graph_breaks* help debugging.... It has been experimented to show 9x% of ..... that is .... 5x% of .....
