...
Apparently, *torch.compile* has the *fullgraph (bool)* parameter. With its default backend (i.e. Inductor), if the argument is *False (default)*,  *torch._dynamo.optimize* searches compileable regions in the function. If set to *True*, the entire function must be capturable as a single graph to avoid an error (i.e. due to graph breaks). While *torch._dynamo.explain* may help debugging, the internals of *torch.compile* can be used independently. For example, symbolic tracing in isolation could capture a form of the code purely for analytical purposes (i.e. not transformation), or code generator may be utilised for programmatically generating models (e.g. based on an arbitrary config file).
