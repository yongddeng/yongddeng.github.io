...
The [word size](http://cpuville.com/Projects/Original-CPU/programming.html) of a CPU refers to the number of bits it can process in a single operation (e.g., 32-bit, 64-bit). It determines the size of registers, the width of the data bus, and the design of the [instruction set architecture](https://www.slideshare.net/slideshow/instruction-set-architecture-254097280/254097280#2) (ISA). Specifically, instructions are a group of commands represented in [binary]() that abstracts various aspects of the hardware and facilitates the software-to-hardware interface. Larger sizes clearly allow the CPU to process more data at once and address larger memory spaces, but indiscriminately increasing the size can lead to higher costs and complexity in circuitry design, making it more challenging to achieve an optimal performance.

For example, the illustrated toy [calculator]() (i.e. 4-bit computer) can merely perform $2^4 = 16$ unique instructions on tiny chunks of data (i.e. 4 bits at a time) ranging between $[0, 15]$ or $[-8, 7]$. As expected, a 32-bit system is by far more powerful as it can define milions of operations and address 4 GB (i.e. or more with segmentation). When programming, using [data types]() that match the CPU's native word length could lead to more efficient computations, but languages like C define the size of its data types only by minimum standards {e.g. *char*: 1 byte, *int*: 4 btyes} to ensure portability across diverse CPU architectures....
...
An instance of a (user-) program, called a [process](), resides in RAM while running. The [text segment](), illustrated in the [memory layout]() of a process, contains the executable code of a program, including instructions (e.g., 4-bit, 32-bit instructions) that the CPU fetches and executes. To ensure program integrity and prevent accidental or malicious modification, this segment is typically marked as read-only. In most modern systems, the region is located at the beginning of the process's virtual address space....

More interestingly, local variables and function parameters are allocated in the [stack]() which operates in a LIFO manner. The stack size is determined at compile time.... The [stack pointer]() (SP) tracks the addresses of [stack frames]() (i.e. frames), that are created by [function calls](https://www.youtube.com/watch?v=cSSpnq362Bk), and being pushed to the [call stack]().... In contrast, dynamically allocating variable-length data (e.g. arrays) in the [heap]() requires sending a [system call]() to the OS to obtain a memory chunk.... While the stack is fixed in size and offers faster memory access via the proximity, heap allocations are more prone to cache misses and fragmentation....
