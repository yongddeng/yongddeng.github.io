...
[Yu et al. (2022)]() introduced continuous batching, leveraging iteration-level scheduling to dynamically manage incoming request, and so reducing latency. Batching inference can increase throughput, but existed systems reserved contiguous memory spaces for KV cache, causing wasted slots and internal-external fragmentations, and thus limiting the size of batches. [Kwon et al. (2023)](https://arxiv.org/abs/2309.06180), inspired by virtual memory and paging, proposed [PagedAttention]() that enables efficient non-contiguous physical memory page on VRAM. They built [vLLM](https://github.com/vllm-project/vllm), an open-source LLM serving system, supporting PagedAttention and other optimisation techniques discussed in this section.
...
- i) fast model execution with CUDA/HIP graph (i.e. compilation); ii) optimized CUDA kernels (e.g. FlashAttention); iii) speculative decoding; iv) quantizations; v) continuous batching; vi) Multi-LoRA; vii) and others;
