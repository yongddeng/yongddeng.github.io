...
[TorchDynamo]() (i.e. Dynamo) serves as the front-end, leveraging PEP 523 (i.e. frame evaluation API) and the [functional transformation]() (FX) framework to represent Python code as high-level SCGs compatible with downstream compilers. For graph acquisition, Dynamo i) [hooks]() into bytecode execution using PEP 523; ii) analyses bytecode and identifies torch.\* and non-torch.\* bits of a program; and iii) convert compilable regions into a graph; *torch.fx.symbolic_trace* can be used with [proxies]() to simulate captured torch.\* operations and record them as nodes in an [FX graph](). A [graph break]() occurs when it encounters operations it cannot support during dynamic tracing.

If a graph break occurs, non-torch.\* (e.g. printing, logging, tensor-dependent conditions) fall back to the default interpreter, bypassing the graph representation. Reducing breaks is indeed important in PT2 (i.e. dynamic-to-static compilation framework) as they degrade performance. Apparently, *torch.compile* has the *fullgraph* parameter that controls the granularity of tracing. If set the default *False*, it attempts to discover compileable regions in the function. When set to *True*, the entire function must be capturable into a single graph. Other APIs, such as *torch._dynamo.explain* and *torch.fx.graph_module.GraphModule.print_tabular*, are helpful for diagnosing breaks.
