...
In fact, PyTorch 1.x rely on the C++ backend because the [eager mode](https://pytorch.org/blog/optimizing-production-pytorch-performance-with-graph-transformations/) execution (which newly defines DCG) is often slow. The Python API acts as the top layer, delegating tasks to C++ modules that manage the autograd engine and tensor operations. Note that [A tensor library]() (ATen) is a wrapper for various low-level C and CUDA libraries: such as [math kernel library]() (MKL), [CUDA deep neural network]() (cuDNN), and [CUDA basic linear algebra subprogram]() (cuBLAS) for compute-heavy lifting. PyTorch backend integrates the autograd engine to track gradients as the libraries lack AD support, and benefits from Python’s flexibility with C++ and CUDA's performance.

[Alban Desmaison](https://pytorch.org/docs/stable/community/persons_of_interest.html) noted: "When Torch7, originally written in [Lua](), rely on the C/C++ libraries: Tensor Handling (TH/THNN) and Tensor Handling CUDA (THC/THCUNN) for CPU/GPU computing, where THC* had CUDA/cuDNN dependencies, i) the Initial PyTorch was a Python wrapper + new autograd engine + TH*; ii) Aten was developed to provide C++ (cpp) interfaces, thus adding functionalities (i.e. tensor operations) to PyTorch can be done in ATen; iii) [LibTorch](), standalone C++ API, was developed as a replacement for the Python wrapper (which used [Pybind11]() for wrapping C++ libraries). LibTorch users can directly exposes the C++ API for high-performances".
