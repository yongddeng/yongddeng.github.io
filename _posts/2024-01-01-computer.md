...
[TorchDynamo]() (i.e. *torch._dynamo*) is the tracer used in [*torch.compile*](), leveraging the [FX]() (i.e. Python-to-Python transformation toolkit) to produce clean graphs that are compatible with downstream compilers. During graph acquisition, TorchDynamo [hooks]() into Python's bytecode execution via CPython's [Frame Evaluation API](). It then uses *torch.fx.symbolic_trace* to simulate operations using [proxies]() and identify capturable regions within a program, that are recoded as nodes of an [FX graph](). Dynamo breaks a computation graph when it encounters unsupported Python features (e.g. data-dependent control flow), and thus delegate it to CPython (i.e. default interpreter).
...
Compared to the behaviour of Python, ... 
...
- E.g. ...
        ...
        # Run dynamo.explain to understand tracing
        explain_output = dynamo.explain(f1, x, y)
        attributes = [attr for attr in dir(explain_output) if not "__" in attr]

        # Access and print the values of each attribute
        ...
        if isinstance(value, list) and len(value) > 0 and hasattr(value[0], "graph"):
            # Special handling for graphs, print tabular representation
...
As the maximum performance gain comes from the minimal graph breaks, one can force TorchDynamo to raise an error upon the first graph break using the try-except block. Apparently, *torch.compile* has the *fullgraph (bool)* parameter. With its default backend (i.e. Inductor), if the argument is *False (default)*,  *torch._dynamo.optimize* searches compileable regions in the function. If set to *True*, the entire function must be capturable as a single graph (i.e. no graph breaks), otherwise it will raise an error (i.e. other backends may stay quite for such issues). *torch._dynamo.explain* may help debugging this Python-level JIT compiler.

- <div style="position: relative; display: inline-block;"> <img src="../assets/blog/2024-01-01-torchdynamo.png" width="500" height="320"> <a href="https://pytorch.org/docs/stable/torch.compiler_dynamo_overview.html" target="_blank" style="position: absolute; bottom: -8px; right: 4px; font-size: 12px; color: white;">[src]</a> </div>
