...
PyTorch (PT), evolved from the [Torch7](), is an open-source deep learning (DL) framework developed by Facebook (FB). TensorFlow 1.x export [static computation graph]() (SCG) from a program for efficient serialisation and deployment, while PT 1.x instead export [dynamic computation graph]() (DCG) for flexibility and easy debugging. [*torch.Tensor*]() class is the core data structure representing homogeneous, multi-dim. rectangular arrays. While its key attributes: [*'requires_grad', 'grad', 'grad_fn'*] enable [automatic differentiation]() (AD) within a DCG, a tensor also contains metadata which describe its shape: [*'size', 'stride', 'dtype', 'device', 'layout'*], highly similar to *numpy.array*.
 
<!-- Torch was implemented in [Lua]() for scientific computing. -->

PT provides various built-in Function classes (e.g. AddBackward0, MulBackward0) for instances and subclasses of *torch.Tensor* (e.g. *nn.Module.weight*, *torch.nn.Parameter*) to enable gradient computation and backpropagation. A new operation can also be defined via [*torch.autograd.Function*](). PT dynamically builds a DCG during executions. *torch.Tensor* instances and *torch.autograd.Function* objects serve as the nodes, while the edges represent data flow and encapsulate the logic of the program. *Tensor.register_hook* may be called during the backward pass to inspect, modify, visualise, or manipulate gradients as they are computed within a neural network (NN).
