...
The existence of Triton and the dominance of PyTorch over TensorFlow might share a common origin (i.e. personal opinion). Compilers are unpleasant to work with even though existing compilers, such as GCC (for C compilation), have undergone decades of refinement. In fact, the typical mental model of ML researchers and practitioners aligns more closely with the behavior of interpreters. Accordingly, [CPython]() (i.e. Python) gained popularity with its dynamic nature, intuitive syntax, and many open-source libraries. 

Note that early interpreters represented source code as [trees]() (i.e. similar to ASTs) and executed code via recursion. However, these trees were primarily designed for representation, not execution, making it difficult to optimise code for higher efficicency and lower them to efficient machine code. This limitation may be clearer with a function in a loop which can easily yield numerous function calls. As a result, a JIT compiler (i.e. mentioned in 3.1) becomes a standard feature of interpreters, enabling faster executaion and runtime optimisations such as register allocation, instruction scheduling, constant folding, dead code and common subexpression elimination.

Modern interpreters generally use a [virtual machine]() (i.e. VM) for JIT compilation. For instance, CPython parses the source code (i.e. .py) into [bytecode]() (i.e. saves .pyc in a \_\_pycache\_\_ directory), a platform-independent intermediate format, and then its stack-based [Python VM]() (i.e. PVM) executes the bytecode (i.e. line-by-line) by interacting with an operating system and hardware. To mitigate significant overhead (and reduce errors) in compilation, JIT compilers focus on "hot regions," or frequently executed parts of the code. The *dis* module in Python can reveal this bytecode produced from the source code (i.e. a chunk of string). 
