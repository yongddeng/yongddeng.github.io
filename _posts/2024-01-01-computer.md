...
The LibTorch was utilised to support [script mode]() via two main APIs. *torch.jit.trace* builds a SCG based on tensor operations executed on fixed inputs during a forward pass, capturing models only with predictable paths (i.e. fixed control flows). Beside the tracer, they added *@torch.jit.script* which compiles input-dependent models directly from Python code into [TorchScript]() IR, thus any TorchScript programs can be saved from Python and loaded into non-Python environments (e.g. train.py $\to$ deploy.cpp). Dynamic models can also be exported to the [Open Neural Network Exchange]() (ONNX) format, though they must comply with ONNX's static graph requirements.
...
- symolically evaluate bytecode
  - [*torch.fx.GraphModule*]()을 arg로 받는 [*torch.dynamo.optimize*]()를 context manager로 두고 타겟 kernel을 감싸는 방식으로.. 그래프 data structure에서 ir로 변환...
