...
Similar hardware specifications can exhibit huge differences in performance. For instance, the use of smaller [transistors]() may reduce power consumption, require lower voltage thresholds, generate less heat, and so ultimately allow for a higher density of transistors to be integrated into the design and enhancing overall performance. Although data access patterns is the most influential factor for the maximum GPU utilisation, architectural innovations are equally important, and industries noted that the numbers of cores were quite sufficient enough for massive computation. What actually causes the bottleneck (in training large models) was the VRAM.
