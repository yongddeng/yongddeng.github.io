...
LibTorch was also utilised to enable [script mode]() via two main APIs. [*torch.jit.trace*](), that aims to compile models with fixed control flows, prepares a SCG using tensor operations executed on fixed inputs during a forward pass. Beside the tracer, [*@torch.jit.script*]() is featured for an input-dependent (i.e. dynamic) model compiliation. It directly lexes and parses Python code and convert them into a TorchScript IR, where any TorchScript programs can be loaded into non-Python environments (e.g. train.py $\to$ deploy.cpp). A dynamic model may be exported to the [Open Neural Network Exchange]() (ONNX) format, though they must comply with ONNX's static graph requirements.
