...
The emergence of Triton and the dominance of PyTorch (i.e. prior to 2.0) over TensorFlow might share a common root cause (i.e. it is a subjective interpretation). Compilers, despite tons of research and know-how, remain difficult to develop and be used, particularly for ML researchers and practitioners who often prefer tools that align with the behavior of interpreters. Perhaps, it explains why [CPython]() (i.e., Python) gained widespread adoption: its dynamic nature, intuitive syntax, and extensive ecosystem of open-source libraries cater to the iterative and experimental workflows. // But they are damn slow...

Modern interpreters integrate a [virtual machine]() (VM) to facilitate JIT compilation. The Python compiler, which is a part of the CPython interpreter, converts frequently executed code paths from source code (i.e. .py) into a platform-independent [bytecode]() (i.e. .pyc). The stack-based [Python VM]() (PVM) then sequentially interpret .pyc at runtime (i.e. line-by-line) by interacting with the OS and hardware, balancing performance and responsiveness. [PEP 3147](https://peps.python.org/pep-3147/) introduced .pyc repository directories, called [\_\_pycache\_\_](), to better organise clutter and enhance cross-version compatibility. The *dis* module can inspect bytecode, revealing the IR generated from source code.
