...
In fact, TS does not support certain libraries nor Python features (e.g. generator, comprehension), but requires incompatible modules to be either wrapped or avoided. To overcome the limitations, the [*torch.compile*]() API was introduced as a domain-specific compiler of [Pytorch 2.0]() (PT2). This distinguishes arbitrary Python code and PyTorch-related code within a program, and represents the latter as a DCG for further optimisations and executions. Technically, *torch.compile* conducts the following procedures: i) graph acquisition (i.e. tracing); ii) graph lowering (i.e. high-level optimisation); and iii) seamless back-end integration along with the prepackaged compilers.
