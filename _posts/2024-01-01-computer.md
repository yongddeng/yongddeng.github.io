...
The LibTorch was further used to enable [script mode]() via two main APIs. [*torch.jit.trace*](), that aims to compile models with fixed control flows, constructs a SCG with tensor operations executed on fixed inputs during a forward pass. Beside the tracer, [*@torch.jit.script*]() is featured for an input-dependent (i.e. dynamic) model. It directly lexes and parses Python code and convert them into a TorchScript IR, such that any [TorchScript]() programs can be loaded into non-Python environments. (e.g. train.py $\to$ deploy.cpp). Dynamic models may be exported to the [Open Neural Network Exchange]() (ONNX) format, though they must comply with ONNX's static graph requirements.
