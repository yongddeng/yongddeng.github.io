...
Subsequently, the [LLVM Core]() takes a role in IR optimisations (e.g. dead code elimination), and a back-end can compile the outputted IR [ahead-of-time]() (AOT) (i.e. *llc* cmd), or execute it directly using a [just-in-time]() (JIT) compilation interpreter (i.e. *lli* cmd). Because the expressive power of an IR and its ability to capture high-level semantics largely determine the sophistication of a compiler, most high-level languages have developed their own AST for associated infrastructures (e.g. Swift-SIL, Rust-MIR, Pytorch-TorchScript). This has led to the unnecessary coexistence of various modules in IRs performing similar tasks, often tailored to specific languages or domain.
