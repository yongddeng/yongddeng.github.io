...
Facebook utilised LibTorch to support JIT compilation via *torch.jit.trace*. It makes a SCG based on tensor operations executed on fixed inputs during a forward pass, capturing models with predictable paths (i.e. fixed control flows). They later introduced *@torch.jit.script*, which directly compiles Python code into [TorchScript]() IR, to support input-dependent models. Notice that any TorchScript program can be saved from Python and loaded in a non-Python environment (e.g. train.py → deploy.cpp). A dynamic model in PyTorch could also be to exported to the [Open Neural Network Exchange]() (ONNX) if they comply with ONNX's static graph requirements.
