...
The main entry point for code compilation in CPython is the *_PyAST_Compile* function in [Python/compile.c](https://github.com/python/cpython/blob/main/Python/compile.c#L1371). It returns a *[PyCodeObject]()* struct that encapsulates the compiled bytecode for a function or block of code. It takes parameters such as i) *co_code*: the sequence of opcodes and operands representing bytecode instructions; ii) *co_consts*: constants (e.g. number, tuple) or nested *PyCodeObject* instances (e.g. function, class, comprehension); iii) *co_names*: variable and attribute names; iv) *co_varnames*: local variable names; v) *co_freevars* / *co_cellvars*: variables used in closures; and vi) *co_filename* / *co_name*: the source file and code block name; 

The CPython uses the [Python virtual machine]() (PVM), a stack-based interpreter, to execute bytecode at runtime by simulating a CPU's execution model (i.e. stack machine). In particular, the (legacy API-) *PyFrame_New* in Objects/frameobject.c creates a [*PyFrameObject*]() which models a [stack frame]() for the code being executed. This object mainly uses i) *[f_code]()*: a reference to the *PyCodeObject*; and ii) *[f_back]()*: a link to the last frame; Consequently, frames form a [linked list]() of *PyFrameObject* pointers, and the core (i.e. default-) function *_PyEval_EvalFrameDefault* in Python/ceval.c serves the main interpreter loop. As per [PEP 523](), this eval function is now configurable.

In summary, CPython relies on a private heap containing "every" Python objects and data structures.  *PyCodeObject* act as a blueprint that can outlive a [function call](), and a *PyFrameObject* holds the [execution context]() for the executing bits. Unlike in other languages, the [call stack]() is a logical stack (i.e. which, conceptually, exists in stack memory) of references. \*\**In recent optimisation, Python 3.11 splits the stack frame object into: the PyFrameObject and the struct [_PyInterpreterFrame](). Moreover, Python 3.12, as proposed in [PEP 709](https://peps.python.org/pep-0709/), inlines list, dict, and set comprehensions into the code instead of compiling them as nested functions that isolate iteration variables*.\*\*
...
The OG interpreters represented source code into ASTs and traversed this tree or a similar IR using recursion. This tree data structure was well-suited for program structure representation, but not optimised for execution. Consequently, types of optimisation were limited, and lowering source code to machine code was also challenging.... compilation to Bytecode and uses of VM improved a little bit, but interpreters are still slow.... [PEP 3147](https://peps.python.org/pep-3147/) introduced .pyc repository directories, called [\_\_pycache\_\_](), to better align clutter and enhance cross-version compatibility. If a .pyc file is available and up-to-date, Python skips the compilation step....
