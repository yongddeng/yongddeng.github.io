...
Consider the 4-bit processor (e.g. calculator) shown below. This can execute only  $2^4 = 16$ unique instructions on small chunks of data (i.e. 4 bits at a time), whilst limiting its representable values to the range $[0, 15]$ or $[-8, 7]$. On the other hand, a 32-bit system is vastly more capable, supporting millions of operations and addressing up to 4 GB of memory (or, even more via advanced techniques). From a programming perspective, using [data types]() that align with the CPU's native word size often results in more efficient computations, but languages including C loosely define data type sizes based on minimum standards, e.g. {int: 4 bytes}, to ensure portability across CPU architectures. Modern CPUs usually use techniques such as [pipelining]() and [branch prediction]() to maximise instruction throughput. 
