...
The [application programming interface]() (API) provided by standard libraries (e.g. libc on Unix-like systems) abstracts the complexity of invoking system calls directly from user mode. That is, when a user-space program requires privileged functionality (e.g. spawning a process), it calls a high-level function via the API, which internally issues one or more [system calls](). These are well-defined entry points into the kernel implemented via software interrupts, trap instructions, or architecture-specific instructions. This controlled interaction ensures that only trusted kernel code (e.g. spawning a process) can access hardware or alter protected system state. <!-- This is typically implemented via a software interrupt, trap instruction, or CPU-specific instruction such as the syscall instruction on x86-64 -->
