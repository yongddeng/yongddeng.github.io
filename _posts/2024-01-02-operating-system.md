...
As systems can execute multiple [cooperating processes](), scheduling should be complemented by [inter-process communication]() (IPC) mechanisms (e.g. pipes, queues, or shared memory buffers) to enable coordination across isolated address spaces. [Shared memory]() allows high-throughput, low-latency communication but requires explicit [synchronisation]() to maintain consistency. Whereas, [message passing]() delegates data transfer to the kernel using abstractions such as pipes, UNIX domain sockets, or System V message queues. The latter is generally slower due to data copying and kernel involvement, but well-simplifies coordination and improves fault isolation.
...
Given that [multiprocessing]() refers to concurrent execution of multiple processes, each with its own isolated address space, distributed across CPU cores, they are coordinated using IPC. For example, PyTorch’s *DataLoader* leverages Python’s *multiprocessing* module to spawn worker processes that load and transform batches concurrently, and it extends shared memory with CUDA memory and IPC to efficiently share GPU tensors across cuda/tensor cores. <!-- shared memory enables low-latency access to NumPy arrays. -->
