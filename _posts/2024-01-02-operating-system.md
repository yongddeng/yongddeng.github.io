...
In networked IPC, processes communicate over [sockets]() identified by [IP addresses]() and [ports](), which act as logical [endpoints]() directing traffic to the correct process. For example, web servers typically use port 80 (HTTP) or 443 (HTTPS), while SSH uses port 22. The OS maps ports to socket endpoints and manages connection queues to deliver [packets]() to the correct process. This concept lies at the centre of distributed systems and ML workloads, as PyTorch uses specific ports (e.g. [MASTER_PORT](https://tutorials.pytorch.kr/intermediate/dist_tuto.html)) during distributed training for both synchronisation and coordination, often over [Transmission Control Protocol]() (TCP) or high-performance backends such as [NCCL](https://docs.nvidia.com/deeplearning/nccl/user-guide/docs/overview.html).
