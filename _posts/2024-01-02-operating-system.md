...
[Multiprocessing]() refers to concurrent execution of multiple processes, typically across CPU cores. Processes on a single core can still run concurrently via [time-slicing]() (a form of multitasking). [Inter-process communication]() (IPC) mechanism, such as pipes, UNIX domain sockets, and shared memory shines since processes are memory-isolated. PyTorch’s *DataLoader* leverages Python’s *multiprocessing* module to spawn worker processes that load and transform batches concurrently. It extends shared memory with CUDA memory and IPC to efficiently share GPU tensors across cuda/tensor cores, while shared memory enables low-latency access to NumPy arrays.
