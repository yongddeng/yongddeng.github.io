...
The 1960s saw the rise of [multiprogramming]() and [time-sharing systems]() (TSS), enabling [concurrent execution]() of programs in memory by having the CPU rapidly switch between them. This shift supported interactive computing and drove the project [Multics](), an ambitious time-sharing system jointly developed by AT&T Bell Labs, GE, and MIT. ____ ______ ______. At Bell Labs in the early 1970s, the [Unix]() was born out of dissatisfaction with the complexity of the Multics. This simpler system built around modular kernel design, hardware abstraction, and multi-user support, while its principles remain central to modern OS design. <!-- The spread of [minicomputers](), including [programmed data processors]() (PDPs), helped popularise OS beyond institutional settings. -->
...
[Ken Thompson]() and [Dennis Ritchie]() at Bell Labs began building a lightweight OS on a spare [PDP-7]() minicomputer in 1969. The system, originally named “Unics” (i.e. a pun on Multics, standing for “Uniplexed Information and Computing Service”), eliminated layers of abstraction and interdependencies, and introduced core design principles such as a [hierarchical file system]() (HFS), segmented memory, dynamic linking, a simple API for system calls, and the idea of separating the kernel from user-level utilities.

A key innovation that set Unix apart was its uniform treatment of input and output. Everything in Unix — files, devices, pipes, and even IPC endpoints — was represented as a file descriptor. This “everything is a file” abstraction, combined with the use of plain text for system configuration and output, made the environment highly scriptable and composable. Small, focused utilities could be chained using pipes to perform complex tasks, encouraging a philosophy of building software as a set of reusable components. This drastically simplified system design and user interaction compared to its predecessors.

As Unix matured, Thompson and Ritchie developed the C programming language to address the need for a portable system-level language. The second major rewrite of Unix in the early 1970s, now in C, allowed it to be compiled and run on a wide range of hardware platforms. This separation from machine-dependent assembly code was revolutionary and made Unix the first widely portable operating system. C and Unix evolved together, with both shaped by the same team at Bell Labs. This tight coupling influenced the conventions of system programming that persist today.

- <div style="position: relative; display: inline-block; background-color: white;"> <img src="https://swcarpentry.github.io/shell-novice/fig/standard-filesystem-hierarchy.svg" width="500" height="250"> <a href="https://swcarpentry.github.io/shell-novice/reference.html" target="_blank" style="position: absolute; top: 0px; left: 4px; font-size: 12px;">[src]</a> </div>

Although AT&T Bell Labs owned Unix, the parent company AT&T was a regulated monopoly under the Communications Act of 1934 and a 1956 consent decree with the U.S. Department of Justice. This agreement prohibited AT&T from entering the commercial computer market, restricting its business to regulated telecommunications services. As a result, Unix was licensed relatively freely to universities and research institutions, most notably the University of California, Berkeley. There, the Berkeley Software Distribution (BSD) introduced enhancements such as the TCP/IP networking stack, job control, and improved virtual memory. These features made BSD a foundation for many commercial Unix variants, including SunOS, HP-UX, and AIX.

However, as vendors introduced their own custom extensions, the Unix ecosystem became increasingly fragmented. To address this, the POSIX (Portable Operating System Interface) standard was developed in the late 1980s by the IEEE. POSIX defined a consistent API and command-line behavior across Unix-like systems, allowing developers to write portable applications without targeting a specific flavor of Unix. Though some incompatibilities remained, POSIX played a key role in unifying the Unix landscape and remains foundational to many modern systems, including Linux and macOS.
...
- <div style="position: relative; display: inline-block; background-color: white;"> <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/cd/Unix_timeline.en.svg/1000px-Unix_timeline.en.svg.png" width="500" height="250"> <a href="https://en.wikipedia.org/wiki/Unix-like" target="_blank" style="position: absolute; top: 0px; left: 4px; font-size: 12px;">[src]</a> </div>
