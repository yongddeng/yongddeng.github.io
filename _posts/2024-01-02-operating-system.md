...
[Threads]() are lightweight execution units within a process that share the same virtual address space. This shared context enables fine-grained parallelism with lower memory overhead and faster context switches compared to processes. Most modern OSes implement the $1 \colon 1$ threading model (e.g. [native POSIX threads library]()) for each user thread to be directly mapped to a kernel thread, while $m \colon 1$ and $m \colon n$ (e.g. [Go]()) models are less widely adopted.  Threads are independently scheduled by the kernel and require synchronisation primitives such as [mutexes](), [spinlocks](), and condition variables to avert [race conditions]() and ensure correct access to shared resources.

In CPython, however, true parallelism across threads has historically been restricted by the Global Interpreter Lock (GIL), which serialises the execution of Python bytecode. Originally introduced to simplify memory management through reference counting, the GIL acts as a mutex, allowing only one thread to execute Python code at a time. While compute-intensive extensions in native code (e.g. NumPy, PyTorch) often release the GIL during execution, the limitation has long hindered multithreaded performance in Python for CPU-bound workloads. Recent efforts, including PEP 703, have introduced a build-time option to remove the GIL starting in Python 3.13, enabling true multithreaded concurrency. This marks a significant step forward for Python’s viability in high-performance and parallel computing contexts.
