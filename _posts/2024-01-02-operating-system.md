...
Advanced CLI-based shells including [Bash](), [Zsh](), and [Fish]() can support scripting, I/O redirection, job control, and process substitution. They parse user commands (e.g. *ls*, *ps*, *cat*), resolves the appropriate binaries, and initiates execution using system calls such as *fork()*, *exec()*, and *wait()*. Note that [terminal emulators]() simply host shell processes and should not be mistaken for the shell itself. On graphical systems, user interaction is instead mediated via desktop environments, such as {Linux: [GNOME](), macOS: [Finder](), Windows: [Explorer]()}, which provide a visual interface while ultimately invoking the same system calls and kernel services underneath.
...
A foundational abstraction in Unix was its uniform treatment of input/output. By representing all I/O resources as [file descriptors](), such as files, devices, IPC endpoints, Unix allowed disparate resources to be accessed via the same read/write interface. This “everything is a file” model, combined with the system’s use of plain-text configuration and output, led the environment very scriptable. Programs inherently were designed as small, single-purpose utilities that could be chained together using [pipes]() (\|). For example, the command *cat log.txt \| grep error \| sort \| uniq -c* reads a log file, filters lines containing “error,” sorts them, and collapses duplicates into counts.
...
- <div style="position: relative; display: inline-block; background-color: white;"> <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/cd/Unix_timeline.en.svg/1000px-Unix_timeline.en.svg.png" width="500" height="310"> <a href="https://en.wikipedia.org/wiki/Unix-like" target="_blank" style="position: absolute;  bottom: -8px; right: 4px; font-size: 12px;">[src]</a> </div>
...
The [CPU scheduler](), a core part of the kernel, manages transitions between the mutually exclusive states and determines which ready process to dispatch next. Classical scheduling algorithms may include i) [round-robin](): fixed time slices; ii) [priority scheduling](): fixed or dynamic priority queues; iii) [multi-level feedback queues]() (MLFQ): dynamically adjusts priorities based on process behaviours. In fact, [context switching]() follows process scheduling in which this incurs overhead from cache disruption, [translation lookaside buffer]() (TLB) flushes, and memory synchronisation. <!-- Modern operating systems such as Linux use the [completely fair scheduler]() (CFS), which maintains a red-black tree to distribute CPU time proportionally by tracking each process’s virtual runtime. -->  

As systems can execute multiple [cooperating processes](), (i.e. for a single or multiple users), scheduling must be supported by [inter-process communication]() (IPC) <!--mechanisms such as pipes, UNIX domain sockets, and shared memory--> to enable coordination across isolated address spaces. For instance, [shared memory]() provides high-throughput, low-latency communication but requires explicit [synchronisation]() to maintain consistency. In contrast, [message passing]() delegates data transfer to the kernel using abstractions such as pipes, UNIX domain sockets, or System V message queues. Although the latter is generally slower due to data copying and kernel involvement, it simplifies coordination and improves fault isolation.

<!-- - <div style="position: relative; display: inline-block;"> <img src="https://applied-programming.github.io/Operating-Systems-Notes/images/processlifecycle.png" width="500"> <a href="https://applied-programming.github.io/Operating-Systems-Notes/2-Process-Management/" target="_blank" style="position: absolute;  bottom: -8px; right: 4px; font-size: 12px;">[src]</a> </div> -->
...
[Multiprocessing]() refers to concurrent execution of multiple processes, each with its own isolated address space, typically distributed across CPU cores. These processes coordinate via IPC mechanisms such as pipes, queues, or shared memory buffers. PyTorch’s *DataLoader* leverages Python’s *multiprocessing* module to spawn worker processes that load and transform batches concurrently. It extends shared memory with CUDA memory and IPC to efficiently share GPU tensors across cuda/tensor cores, while shared memory enables low-latency access to NumPy arrays. Note that processes on a core can still run concurrently under appropriate time-slicing.
...
- <iframe width="500" height="280" src="https://www.youtube.com/embed/Y2mDwW2pMv4?si=OoTcq4AeQHU8Ibw2" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

<!-- [Threads]() are lightweight execution units within a process that share the same address space. It enables direct access to shared code, heap, and global variables, enabling fine-grained parallelism with lower memory and context-switching overhead than processes. Most operating systems implement the $1:1$ threading model (e.g. Linux NPTL), where each user thread corresponds to a kernel thread, while alternative $m:1$ and $m:n$ models are less commonly chosen. Threads are independently scheduled by the kernel and require synchronisation primitives (e.g. mutual exclusion locks (i.e. mutexes), spinlocks, and condition variables) to avoid race conditions. (i.e. scheduling can be preemptive or cooperative, and mismanaged synchronisation can result in race conditions or deadlocks). -->


[Threads]() are lightweight execution units within a process that share the same address space. This allows direct access to shared code, heap, and global variables, enabling fine-grained parallelism with lower memory usage and context switching overhead compared to processes. Most operating systems implement the $1:1$ threading model (e.g., Linux NPTL), where each user thread maps directly to a kernel thread. Alternative models, such as $m:1$ (many user threads mapped to one kernel thread) and $m:n$ (many user threads multiplexed over many kernel threads), are less commonly adopted. Notably, the [Go]() employs an $m:n$ model: its runtime maps many lightweight user-level goroutines to a smaller number of system threads, allowing efficient concurrency with low scheduling overhead. 

Threads are independently scheduled by the kernel and require synchronisation primitives, such as [mutual exclusion locks]() (mutexes), [spinlocks](), and condition variables, to prevent [race conditions]() and ensure correct access to shared resources. [PEP 703]() introduces a build-time option to disable the [global interpreter lock]() (GIL), which has constrained [multithreaded]() performance by serialising the execution of Python bytecode across threads as a mutex, thereby limiting the effectiveness of threading for CPU-bound operations such as matrix multiplication. This development marks a significant step toward enabling native multithreaded concurrency in Python 3.13+.

<!-- https://velog.io/@soopsaram/Kernel-Mutex%EC%99%80-Spinlock-%EC%9D%B4%ED%95%B4%ED%95%98%EA%B8%B0 -->

 
- Python developers often rely on multiprocessing for parallel execution, supported by high-level libraries such as *joblib*, *torch.multiprocessing*, and *ray*, which enable efficient process spawning and IPC. 

- Many ML libraries perform compute-intensive operations in native C/C++ code that runs outside the Python interpreter. During these operations, the GIL is temporarily not held, which allows other Python threads to make progress in parallel, enabling limited thread-level concurrency despite Python’s usual restrictions.

- [Numba]() allowed *nogil* option
