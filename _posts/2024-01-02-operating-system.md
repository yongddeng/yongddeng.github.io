...
Advanced CLI-based shells (e.g. Bourne Again Shell - [Bash](), [Zsh](), and [Fish]()) support scripting, I/O redirection, job control, and process substitution. They parse user commands (e.g. *ls*, *ps*, *cat*), resolves the appropriate binaries, and initiates execution using system calls such as *fork()*, *exec()*, and *wait()*. [Terminal emulators]() (e.g. macOS Terminal, GNOME Terminal) merely host shell processes, and thus, should not be confused with the shell itself. Likewise, graphical environments - {Linux: [GNOME](), Windows: [Explorer](), macOS: [Finder]()}, though visually distinct, offer a user-friendly layer that interfaces with the same underlying system calls and kernel services.
