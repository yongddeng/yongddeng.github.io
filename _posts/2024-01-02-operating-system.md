...
Each process executes within its own isolated virtual [address space](), comprising distinct code, data, heap, and stack segments. This isolation ensures protection between processes and underpins system stability. The OS kernel maintains per-process metadata via a structure, known as the [process control block]() (PCB), that contains its identifiers (PID/PPID), execution state, CPU register, memory mappings, scheduling parameters, and open file descriptors. Each process lives a [process life cycle](): new (creation), ready (queued for CPU), running (actively scheduled), waiting (blocked on I/O, synchronisation, or an event), and terminated (completed or killed). 

The [CPU scheduler](), a core kernel component, handles transitions between mutually exclusive process states and selects which ready process to run next. i) [Round-robin](): fixed time slices; ii) [priority scheduling](): fixed or dynamic priority queues; iii) [multi-level feedback queues]() (MLFQ): dynamically adjusts priorities based on CPU or I/O behaviour; are classical scheduling algorithms. <!-- Modern operating systems such as Linux use the [completely fair scheduler]() (CFS), which maintains a red-black tree to distribute CPU time proportionally by tracking each process’s virtual runtime. --> Each scheduling decision triggers a [context switch]() where the CPU saves the current process state and loads the next. Though vital for multitasking, it incurs overhead from cache disruption, [TLB flushes](), and memory synchronisation, making efficient scheduling crucial to efficiency.

- ...

[Multiprocessing]() refers to the concurrent execution of multiple processes across CPU cores or via time-slicing on a single core. This is widely used to harness parallelism in compute-intensive domains. Since processes are memory-isolated, [inter-process communication]() (IPC) such as pipes, UNIX domain sockets, and SMEM is useful. For instance, PyTorch’s *DataLoader* uses Python’s *multiprocessing* module to spawn worker processes that load and transform batches concurrently. In particular, SMEM enables low-latency access to shared data such as CUDA tensors or NumPy arrays, while [message passing]() offers a safer abstraction for structured communication.

In networked IPC, processes communicate over [sockets]() identified by [IP addresses]() and [ports](), which act as logical [endpoints]() directing traffic to the correct process. For example, web servers typically use port 80 (HTTP) or 443 (HTTPS), while SSH uses port 22. The OS maps ports to process identifiers and manages connection queues to ensure correct packet delivery. The concept also plays a crucial role in distributed systems and ML workloads, where PyTorch uses specific ports (e.g. [MASTER_PORT](https://tutorials.pytorch.kr/intermediate/dist_tuto.html)) for synchronisation and/or coordination (i.e. in distributed training) over [Transmission Control Protocol]() (TCP) or high-performance transport layers such as [NCCL](https://docs.nvidia.com/deeplearning/nccl/user-guide/docs/overview.html).

- ...

[Threads]() are lightweight execution units within a process that share the same address space. This allows direct access to shared code, heap, and global variables, enabling fine-grained parallelism with lower memory and context-switching overhead than processes. Most OSs implement the 1:1 threading model (e.g. Linux NPTL), where each user thread corresponds to a kernel thread, and the alternatives such as M:1 and M:N models are less commonly used. Threads are independently scheduled by the kernel and require synchronisation primitives (e.g. mutexes, spinlocks, and condition variables) to avoid race conditions. <!-- (i.e. scheduling can be preemptive or cooperative, and mismanaged synchronisation can result in race conditions or deadlocks). -->

Python 3.13 introduces a build-time option to disable the [Global Interpreter Lock]() (GIL) and achieve native [multithreaded]() concurrency in future Python-based workloads. Previously, the GIL serialised execution of Python bytecode across threads, and rendered Python threads ineffective for CPU-bound tasks such as training loops. Instead, Python developers often use multiprocessing for parallel execution, aided by libraries such as *joblib*, *torch.multiprocessing*, and *ray* offer high-level interfaces for spawning processes and efficiently sharing data. Nonetheless, many ML libraries release the GIL during compute-critical C/C++ operations, allowing limited thread-level parallelism despite Python’s restrictions.
