...
{% comment %}
Arc: from bits to specifications — label, hide, formalise, realise, compose; then when the rules are checked.
  p1: problem — bit patterns mean nothing untyped; types declare domain and operations, exposed internals broke at scale
  p2: ADT — Liskov's specification vs implementation; CLU enforces hiding, correctness/modularity/substitutability
  p3: foundation — ADT as algebraic structure (S, {o_i}, A); theorems from axioms alone, proved once, hold everywhere
  p4: realisation — int32 as Z/2^32, str and list as free monoids; finite approximations with hidden storage
  p5: composition — product, sum, function types; functors, Curry-Howard, programs as proofs
  p6: discipline — static/dynamic x strong/weak; where bugs surface
  p7: inference — Hindley-Milner dissolves safe-vs-verbose; gradual typing retrofits hints
{% endcomment %}
