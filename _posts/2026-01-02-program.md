...
The [transformer]() architecture (Vaswani et al., 2017) made that persistent graph worth having, the need urgent on three counts and none met by eager's one precompiled kernel per operation, i) throughput: its elementwise kernels are memory-bound (§601#2.3), as *(a + b) * c* writes the sum to HBM and reads it straight back for the product, a round trip that only a compiler seeing both can fuse away by keeping the sum in SRAM ([He, 2022]()); ii) deployment: the artefact must carry neither Python nor a GIL which serialises a process's threads; iii) portability: one graph must retarget to accelerators no hand-written kernel would reach.

Billed as the bridge to production, [TorchScript]() (PyTorch 1.0, 2018) captures the model AOT as a persistent SCG written in TorchScript IR, as [*torch.jit.trace*]() tapes the path a mock input exercises and [*torch.jit.script*]() parses the source to preserve control flow. Either serialises to a *.pt* archive, yet the model as a whole never reaches machine code despite the *torch.jit* namespace. Execution then falls to [LibTorch](), PyTorch's Python-free C++ distribution, whose interpreter walks the graph node by node into the same precompiled kernels, the way CPython walks bytecode. The attempt tackled deployment alone, but rewriting a program<!-- rewritten: NumPy calls to tensor ops, dynamic containers to nn.ModuleDict, type annotations added --> into its typed Python dialect cost eager's ease.
...
- <div style="display: inline-block;"> <iframe src="../assets/blog/ir-ladder.html" width="665" height="205" style="border: none; overflow: hidden;" scrolling="no"></iframe> <div style="font-size: 11px; font-style: italic; color: #666; margin-top: 5px;">The 1.x stack twice, its components on the left and one source line descending every altitude on the right.</div> </div>
...
[torch.compile]() (PyTorch 2.0, 2023) moved capture into the run, a tracing JIT comprising front-end, IR, and back-end.<!-- trim: (i.e. compiles what it executes) --> [TorchDynamo](), as front-end, replaces CPython's frame-evaluation pointer (PEP 523), walks the bytecode, and extracts tensor operations into an [FX graph]() (Reed et al., 2022) in [Torch IR](). The run produces many guarded subgraphs (i.e. partial SCGs) rather than the one whole graph TorchScript demanded. The SCGs descend through the back-end to machine code, winning the throughput graph-walking never did, for training and inference alike. Nothing need be rewritten, as the untraceable is handed back rather than rejected, eager's price refunded.<!-- trim: which is the whole of TorchScript's price refunded -->
...
The other bound is what the trace assumed, recorded as [guards]() (e.g. dtypes, shapes) rechecked on every later call. An unseen shape fails its guard and forces a retrace. The second shape is what teaches the compiler because automatic dynamic marks the dimension that moved and retraces it with a symbolic size (*SymInt*) that later shapes satisfy. [Dynamic shapes]() may instead be declared up front to skip the wasted first pass. The tolerance is finite, since a frame recompiled past *cache_size_limit* (eight by default) is abandoned to the interpreter permanently, the speculation and deoptimisation of any JIT replayed on tensors.
...
Past both bounds, an SCG taped from the forward alone enters the back-end. [AOTAutograd]() derives the missing backward, running fake tensors (shapes without data) through eager's own differentiation rules, and splits the joint trace into forward and backward FX graphs. During the trace, both passes are decomposed into ~250 primitives of [ATen](), PyTorch's "A Tensor Library" built in C++ over the vendor kernels of MKL, cuDNN, and cuBLAS. The back-end thus receives the pair in ATen IR, not yet executable and deliberately pessimised (e.g. eager's fused *softmax* outruns its separated *exp*, *sum*, and *divide*), but the stable contract between capture and code generation.<!-- trim: Holding both passes, the compiler optimises them jointly (e.g. recomputing a cheap activation over storing it). -->

{% comment %}
torch.compile in use. No mock input at compile time, the real first batch is the capture.

model = MyModel()
model = torch.compile(model)   # nothing happens yet, no input needed

y = model(x)                   # first call: Dynamo captures with THIS x,
                               # AOTAutograd + Inductor compile, then it runs
y = model(x2)                  # guards pass -> cached machine code
y = model(x_odd_shape)         # a guard fails -> retrace for the new shape
loss = criterion(y, target)
loss.backward()                # runs the compiled backward from AOTAutograd

Knobs: mode="reduce-overhead" (CUDA graphs), mode="max-autotune" (Inductor
benchmarks kernel variants), fullgraph=True (error on any graph break),
dynamic=True (symbolic shapes up front), backend="inductor" is the default.
Also works as a decorator, @torch.compile, on a plain function.

Contrast: torch.jit.trace(model, example_input) demanded the mock input at
compile time. torch.compile takes none, capture waits for the run.
{% endcomment %}

The default back-end [TorchInductor]() lowers the ATen pair into its own loop-level IR to emit Triton for GPUs and C++/OpenMP for CPUs. Each primitive is spelt out over its elements (e.g. the *sum* becomes a loop of additions) with optimisations (e.g. loop tiling to fit an SM's SRAM). In particular, [kernel fusion]() folds chains of the decomposed primitives into one kernel to minimise HBM traffic<!-- trim: across boundaries no precompiled kernel could cross -->, when every operation in the chain is pointwise.<!-- trim: , which *softmax*'s normalisation is not, every output depending on a whole row --> That is, Inductor generates the rest, but delegates the *aten::scaled_dot_product_attention* node<!-- trim: , kept whole through the descent, --> (as the matmul to cuBLAS) to attention kernels, usually FlashAttention's (§601#2.3), whose hand-derived algebra fuses *softmax*'s normalisation.<!-- trim: as no compiler could; fused through the normalisation beyond any compiler's search -->

{% comment %}
Inductor's optimisations on y = torch.relu(x * 2 + b), three pointwise ops.

Eager, one precompiled kernel per op, each a round trip through HBM:
  tmp1 = mul_kernel(x, 2)      # read x, write tmp1
  tmp2 = add_kernel(tmp1, b)   # read tmp1 back, write tmp2
  y    = relu_kernel(tmp2)     # read tmp2 back, write y
mul_kernel cannot contain the add, the boundary is fixed at build time.

Kernel fusion, one generated loop does all three per element:
  for i in range(n):
      y[i] = max(x[i] * 2 + b[i], 0.0)   # tmp1, tmp2 never touch memory

Loop tiling, the loop reorganised into cache-sized blocks:
  for i0 in range(0, n, 64):
      for i in range(i0, i0 + 64): ...

Memory planning, buffers reused across fused groups instead of fresh allocations.

Reassembly across boundaries: for softmax(x) * scale, eager runs the fused
softmax kernel then a mul kernel, the multiply can never enter softmax's
precompiled kernel. Inductor holds the primitives (exp, sum, div) as loose
loops and fuses the div with the mul, a boundary no kernel library offered
because the library's unit was softmax, not its pieces.
{% endcomment %}

The first *y_pred = model(x)* after *model = torch.compile(model)* pays a warm-up bill of tracing and compilation that deployment settles ahead of time. [torch.export]() captures the whole program as one graph without Python fallback, [AOTInductor]() compiles it into a shared lib. serving without the interpreter, and [ExecuTorch]() carries it onto mobile.<!-- trim: Together they retire TorchScript's last deployment role. --> A pluggable back-end such as TensorRT or XLA registers against only the 250 primitives, so the stack stays portable under JIT compilation.<!-- trim: the exits are parallel, one per toolchain: [torch-mlir]() routes the graph into MLIR's Torch dialect for hardware the stack has never met, while AWS Inferentia and Trainium instances accept only what their Neuron compiler emits (fed via the XLA path, torch-neuronx capturing to HLO), capture there a precondition rather than an optimisation. --> [Ansel et al., 2024](https://dl.acm.org/doi/10.1145/3620665.3640366) also reported geometric-mean speedups of 2.27x for inference and 1.41x for training on an A100 across 180+ models, each from the one-line change, so the throughput settled last.
