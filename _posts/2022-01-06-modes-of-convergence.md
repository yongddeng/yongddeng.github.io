...
The source of knowledge consented some abuse of notation. In particular, given a limiting random variable $X := X\_{\infty} \sim \mathcal{N}(0,1)$, we are allowed to denote $X_n \xrightarrow{d} X$ rather than explicitly writing $X_n \xrightarrow{d} X$ as $n\to\infty$. It however emphasised that only the continuity points of $F_X$ should be considered for the distributional convergence. I.e. if $(X\_n)\_{n\in\mathbb{N}}$ is a sequence consists of $X_n = n^{-1}$ and $X_n \xrightarrow{p} X := 0$ (i.e. $X$ is [degenerate]()), then $X_n \not\xrightarrow{d} X$ for all $x \notin C(F_X)$. In other words, if $F_{X_n}(x)$ is a sequence of distribution functions such that $F\_{X\_n}(x)=0$ for $x < n^{-1}$ and $F\_{X\_n}(x)=1$ for $x \geq n^{-1}$, then $F\_{X\_n} \to F_X$ as $n\to\infty$ only when $x\neq0$ (i.e. $P(X=0) = 1$).

If $(X\_n)\_{n\in\mathbb{N}}$ converges completely, almost surely, in probability, in $r$-mean, or in distribution to $X$, then $X$ preserves the uniqueness. Especially, for the preceding four modes, if $X\_n \to X$ and $X\_n \to Y$$, then $X = Y$ almost surely. However, if $X\_n \to X$ and $X\_n \to Y$ in distribution, then $F\_{X}(x)=F\_{Y}(x)$ for all $x \in C(F\_{X})$. In particular, for all $x \in C(F\_{X}) \cap C(F\_{Y})$, the triangle inequality shows $\vert F\_X - F\_Y \vert \leq \vert F\_X - F\_{X\_n} \vert + \vert F\_{X\_n}-F\_Y \vert \to 0$ as $n\to\infty$. If $(X\_n)\_{n\in\mathbb{N}}$ and $(Y\_n)\_{n\in\mathbb{N}}$ converge almost surely, in probability, or in $r$-mean, then $aX\_n + bY\_n \to aX + bY$ for any $a,b\in\mathbb{R}$ correspondingly to each mode of convergence **(#1)**.
...
The firt implication in $(1)$ follows from the 1st Borel-Canteli lemma. In fact, if $X_n$ are ind. and $X$ is degenerate, then the converse is true due to the 2nd Borel-Canteli lemma. The relation $(2)$ follows from Markov's inequality that bounds $P(\vert Y_n \vert > \varepsilon) \leq \varepsilon^{-r}\operatorname{E}{\vert Y_n \vert}^r$, where $Y_n = X_n-X$. If a weakly convergence sequence $({\vert X_n \vert}^p)\_{n\in\mathbb{N}}$ is [uniformly integrable]() (u.i.), then we hold $X_n \xrightarrow{p} X \Longrightarrow X_n \xrightarrow{r} X$. Specifically, a sequence $(X_n)\_{n\in\mathbb{N}}$ is u.i. if for all $\varepsilon > 0$ there exists $B_\varepsilon \in [0, \infty)$ such that $\sup_{n} \operatorname{E}({\vert X_n \vert}I\_{\vert X_n \vert > B_\varepsilon}) \leq \varepsilon$ uniformly in $n\in\mathbb{N}$. Whlst it is equal to have $\lim\_{b\to\infty} \sup_n \operatorname{E}({\vert X_n \vert}I\_{\vert X_n \vert > b})=0$ **(#2)**, the below are some criteria for uniform integrability of a sequence.

$X$ is u.i. if and only if $\operatorname{E}\vert X \vert < \infty$. If we let $\operatorname{E}\vert X \vert = \operatorname{E}({\vert X \vert}I_{\vert X \vert \leq b}) + \operatorname{E}({\vert X \vert}I_{\vert X \vert > b})$, then ${\vert X \vert}I_{\vert X \vert \leq b} \geq 0$ and ${\vert X \vert}I_{\vert X \vert \leq b} \nearrow \vert X \vert$ as $b\to\infty$. The MCT yields $\lim_{a\to\infty}\operatorname{E}({\vert X \vert}I_{\vert X \vert \leq b}) = \operatorname{E}(\lim_{a\to\infty}{\vert X \vert}I_{\vert X \vert \leq b}) = \operatorname{E}{\vert X \vert}$, and hence $\lim_{a\to\infty}\operatorname{E}({\vert X \vert}I_{\vert X \vert > b}) = 0$ for any $\operatorname{E}{\vert X \vert} < \infty$. Moreover, $(X\_n)\_{n\in\mathbb{N}}$ is u.i. if and only if $(X\_n)\_{n\in\mathbb{N}}$ holds (i) boundness: $\sup_n\operatorname{E}{\vert X_n \vert}<\infty$ **(#3)**; (ii) [uniform continuity](): for all $\varepsilon > 0$ there exists $\delta > 0$ and so for all $A\in\mathcal{F}$ with $P(A) \leq \delta$ the value $\sup_n \operatorname{E}({\vert X_n \vert} I_A) = \sup_n\int_{A}{\vert X_n \vert}\,\mathrm{d}P \leq \varepsilon$. We may write $\sup_n\operatorname{E}{\vert X_n I_A \vert} \to 0$ as $P(A) \to 0$;. Note that $\operatorname{E}({\vert X_n \vert}I_A) \leq aP(A) + \operatorname{E}({\vert X_n \vert} I\_{\vert X_n \vert >a}) \leq a\delta + \varepsilon/2 < \epsilon$, so we may let $A_n = \lbrace \vert X_n \vert > a \rbrace$ and use Markov's inequality to prove the necessity.

The relation $X_n \xrightarrow{p} X \Longrightarrow X_n \xrightarrow{r} X$ under the uniform integrability is a generalisation of the [Vitali convergence theorem]() (which is a generalisation of the DCT). Measure theoretically, it states that a sequence $(f\_n)\_{n\in\mathbb{N}} \subset L^p$ on a measure space $(X, \Sigma, \mu)$ with $\mu(X)<\infty$ converges in $L^p$ to $f \in L^p$ if and only if (i) $f_n$ converges in $\mu$-measure to $f$ **(#4)**; (ii) $({\vert f_n \vert}^p)\_{n\in\mathbb{N}}$ is u.i.; A probability measures $P$ is certainly finite since $P(\Omega)=1$. In fact, the finite measure supposition can be relaxed if, in addition, (iii) for all $\varepsilon>0$ there exists $E\_{\varepsilon} \in \Sigma$ such that $(F\in\Sigma) \land (F\cap{E_\varepsilon}) = \emptyset$ and $\sup_n\int_{F}{\vert f_n \vert}^p\,\mathrm{d}\mu < \varepsilon^p$. If we let $E_\epsilon = X$ whenever $\mu(X)<\infty$, then $F=\emptyset \in \Sigma$ is the only set such that $E_\varepsilon \cap F =\emptyset$ and $\int_\emptyset {\vert f_n \vert}^p = 0$ **(#5)**.
