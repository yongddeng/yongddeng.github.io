...
If $(X\_n)\_{n\in\mathbb{N}}$ converges completely, almost surely, in probability, in $r$-mean, or in distribution to $X$, then $X$ is unique. Especially, for the preceding four modes, if $X\_{n} \to X$ and $X\_{n} \to Y$, then $X = Y$ almost surely. However, if $X\_{n} \to X$ and $X\_{n} \to Y$ in distribution, then $F\_{X}(x)=F\_{Y}(x)$ for all $x \in C(F\_{X})$. In particular, for all $x \in C(F\_{X}) \cap C(F\_{Y})$, the triangle inequality shows $\vert F\_{X} - F\_{Y} \vert \leq \vert F\_{X} - F\_{X\_n} \vert + \vert F\_{X\_n}-F\_{Y} \vert \to 0$ as $n\to\infty$. If $(X\_{n})\_{n\in\mathbb{N}}$ and $(Y\_{n})\_{n\in\mathbb{N}}$ converge almost surely, in probability, or in $r$-mean, then $aX\_{n} + bY\_{n} \to aX + bY$ for any $a,b\in\mathbb{R}$ correspondingly to each mode of convergence **(#1)**.
