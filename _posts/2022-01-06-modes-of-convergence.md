...
If $(X\_n)\_{n\in\mathbb{N}}$ converges completely, almost surely, in probability, in $r$-mean, or in distribution to $X$, then $X$ preserves the uniqueness. Especially, for the preceding four modes, if $X_n \to X$ and $X_n \to Y$$, then $X=Y$ almost surely. However, if $X_n \to X$ and $X_n \to Y$ in distribution, then $F_X(x)=F_Y(x)$ for all $x \in C(F_X)$. In particular, for all $x \in C(F_X) \cap C(F_Y)$, the triangle inequality shows $\vert F_X - F_Y \vert \leq \vert F_X - F\_{X\_n} \vert + \vert F\_{X\_n}-F_Y \vert \to 0$ as $n\to\infty$. If $(X\_n)\_{n\in\mathbb{N}}$ and $(Y\_n)\_{n\in\mathbb{N}}$ converge almost surely, in probability, or in $r$-mean, then $aX_n + bY_n \to aX + bY$ for any $a,b\in\mathbb{R}$ correspondingly to each mode of convergence **(#1)**.
