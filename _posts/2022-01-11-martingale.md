...
A few adjustments of the definition induces another accessible martingales. We let the [martingale differences]() (i.e increments) be $D\_{0} = X\_{0}$ and $D\_{t} = X\_{t} - X\_{t-1}$ for all $t \in \mathbb{Z}^{+}$ **(#2)**, so that $X\_{t}$ is the sum of $D\_{s}$ for all $s \leq t$. An adapted stochastic process $D = (D\_{t})\_{t \in T}$ will be a [martingale difference sequence]() if, for all $s, t \in T$ and $0 \leq s \leq t$, it satisfies both (i) $D\_{t} \in L^1$; (ii) $\operatorname{E}(D\_{t} \,\vert\, \mathcal{F}\_{s}) = 0$ a.s.; Preferably, it is trivial that $\operatorname{E}D\_{t} = 0$ for all $t \in \mathbb{Z}^{\geq}$, and also we can prove $\operatorname{E}(D\_{t} \,\vert\, \mathcal{F}\_{s}) = 0$ a.s. for all $t, s \in \mathbb{Z}^{\geq}$ with $s < t$. In other words, a stochastic process $X$ with ind. increments $D\_{t}$ is a martingale with mean zero whilst $X$ and $D$ are adpated to the common $F$.
