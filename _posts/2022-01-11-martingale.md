...
A few adjustments in the definition yields another accessible martingales. Specifically, let the [martingale differences]() (i.e increments) be $D\_{0} = X\_{0}$ and $D\_{t} = X\_{t} - X\_{t-1}$ for all $t \in \mathbb{Z}^{+}$ **(#2)**, so that $X\_{t}$ is the sum of $D\_{s}$ for all $s \leq t$, and $X$ and $D$ are adpated to the common $F$. 


Furthermore, as it is almost trivial that $\operatorname{E}D\_{t} = 0$ for all $t \in \mathbb{Z}^{\geq}$, we can prove that $\operatorname{E}(D\_{t} \,\vert\, \mathcal{F}\_{s}) = 0$ a.s. for all $t, s \in \mathbb{Z}^{\geq}$ such that $s < t$, and we can have some intuition that a sequence with ind. increments would be a martingale with zero mean.
